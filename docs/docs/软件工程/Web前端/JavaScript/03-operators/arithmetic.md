---
title: "算术运算符"
description: "运算符是处理数据的基本方法，用来从现有的值得到新的值。JavaScript 提供了多种运算符，覆盖了"
---
...
算术运算符对数字运算；但 + 遇到字符串会拼接，导致 `'1'+1` 为 `'11'`。其余算术运算符会把操作数 coerce 为数字。
...
除 + 外算术运算会转数字；+ 遇字符串变拼接，注意操作数类型。

## 练习

动手检验一下自己：

1. 预测 `'1'+2`、`1+2`、`1-'2'` 三个表达式的结果。
2. 用 % 判断一个整数是奇数还是偶数。
