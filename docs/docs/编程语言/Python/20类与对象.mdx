...
- 掌握类、实例、属性、方法的定义与 `self` 的本质
- 理解继承、MRO（方法解析顺序）与多重继承的菱形问题
- 会用特殊方法（魔术方法）自定义类型转换与运算符重载，了解元类的使用边界
...
- [Python · 10函数](../10函数)
- 对照阅读：[Java · 面向对象与核心语法](../../Java/语言基础/面向对象与核心语法)（静态类型的 OOP 实现）
- 抽象机制对照：[Rust · 泛型与Trait](../../Rust/类型系统/泛型与Trait)

## 核心概念

Python 的 OOP 建立在「一切皆对象」之上：类本身也是对象（由元类创建），实例方法只是把实例作为第一个参数传入的普通函数，这就是 `self` 需要显式声明的原因。特殊方法（`__init__`、`__len__`、`__add__` 等）是 Python 的「协议」机制——你不需要继承任何接口，只要实现对应的特殊方法，内置语法就会对你的类生效（鸭子类型）。

本章主线：
- 类与实例、属性与方法、封装约定
- 继承、MRO 与 `super()`
- 特殊方法：构造、表示、容器协议、上下文管理
- 类型转换与运算符重载、元类编程
...
`super(class, instance)` 函数用于在子类中调用父类（或超类）的方法，特别是支持多继承的场景下，它会根据方法的解析顺序[Method Resolution Order, MRO](#mro-method-resolution)来决定调用查找类的下一个类的方法。
...
### `__mro__`与mro()与`__bases__` {#mro-method-resolution}
...
类是对象的模板，特殊方法是接入语言语法的插口，MRO 决定多重继承下方法从哪来。元类威力大但很少需要——绝大多数场景用类装饰器或 `__init_subclass__` 就够了。

## 练习

动手检验一下自己：

1. `self` 是关键字吗？如果把它改名为 `this`，代码还能运行吗？为什么？
2. 给出一个菱形继承的例子，用 `__mro__` 说明方法查找顺序。
3. 实现一个类，使其支持 `len(obj)`、`obj[0]` 和 `with obj:` 三种语法，分别需要哪些特殊方法？

## 易错点

- `self` 必须显式写出：方法第一个参数 `self` 不是关键字，漏写会在调用时报参数数量不符。
- 可变类属性是共享的：`class C: x = []` 让所有实例指向同一列表，实例级数据应写 `self.x = []`。
- 子类忘了调父类：`super().__init__()` 漏写会导致父类的初始化逻辑不执行。
- 名称改写：`__foo` 是私有改名（name mangling），子类访问不到；约定上的私有用单下划线 `_foo`。
- 自定义相等要同步 `__hash__`：实现 `__eq__` 后放进 set/dict 需要同时定义 `__hash__`，否则不可哈希。

## 延伸阅读

- 上一篇：[Python · 10函数](../10函数)
- 下一篇：[Python · 25迭代器](../25迭代器)
- 返回 [Python 语言总览](../)
- 跨语言对照：[Go](../../Go/) · [Rust](../../Rust/) · [C](../../C/)
- 跨主题关联·抽象：[设计模式](../../../软件工程/工程实践/设计模式)（Python 不需要 interface 关键字，用鸭子类型与 ABC 表达同样的抽象）
- 跨语言抽象对照：[Java 接口与多态](../../../编程语言/Java/语言基础/面向对象与核心语法)（静态类型的显式契约）· [JavaScript 原型继承](../../../软件工程/Web前端/JavaScript/oop/prototype)（同为动态语言，但用原型链而非类继承树）
