...
sidebar_position: 3
title: 网页自动化
description: "网页自动化实战：Selenium/Playwright 等框架选型，XPath 与 CSS 选择器数据提取，属性与正则提取、命名空间处理与组合使用"
---
...
- 了解主流自动化框架（Selenium / Playwright 等）的定位与差异
- 掌握 XPath、CSS 选择器与正则三种数据提取方式及其组合使用
- 能处理命名空间、动态渲染等实际抓取中的常见障碍

## 前置知识

在阅读下面的内容前，建议先掌握：

- [Python · 2字符串](../../../编程语言/Python/2字符串)（正则与文本处理）
- [接口测试](../接口测试)（能直接请求接口时无需驱动浏览器）

## 核心概念

网页自动化有两条技术路线：**直接请求接口**（快、稳、省资源）和**驱动真实浏览器**（能执行 JS、能处理复杂交互，但慢且脆弱）。选择依据只有一条——目标数据是否由前端 JS 动态生成。定位元素时，CSS 选择器简洁、XPath 表达力强（支持向上查找与文本匹配）、正则则用于从非结构化文本里兜底提取。

本章主线：
- 自动化框架选型
- 数据提取：XPath 与 CSS 选择器
- 提取文本、属性与正则兜底
- 命名空间处理与组合使用
...
[BS4 解析库用法详解](https://c.biancheng.net/python_spider/bs4.html)
...
<a href="https://c.biancheng.net/python/" id="link1">python教程</a>
<a href="https://c.biancheng.net/c/" id="link2">c语言教程</a>
"""
...
<a href="https://c.biancheng.net/python/" id="link1">python教程</a>,
<a href="https://c.biancheng.net/c/" id="link2">c语言教程</a> and
"""
...
<a href="https://c.biancheng.net/python/" id="link1">python教程</a>
<a href="https://c.biancheng.net/c/" id="link2">c语言教程</a>
<a href="https://c.biancheng.net/django/" id="link3">django教程</a>
...
<a href="https://c.biancheng.net/view/8066.html" id="link5">关于网站</a>
<a href="https://c.biancheng.net/view/8092.html" id="link6">关于站长</a>
</p>
"""
from bs4 import BeautifulSoup
soup = BeautifulSoup(html_doc, 'html.parser')
...
sel.xpath("//a:entry/a:author/g:image/@src", namespaces={"a": "https://www.w3.org/2005/Atom","g": "http://schemas.google.com/g/2005"}).getall()
```
...
上文用 requests + BeautifulSoup 解析静态页，遇到 JS 渲染的页面就需要无头浏览器。下面用 Playwright 完成「打开页面 → 填表 → 提交 → 等结果」的完整闭环：

```python
# 用 Playwright 做无头浏览器自动化（处理 JS 渲染、表单、点击）
...
with sync_playwright() as p:
    ...
    title = page.title()
    page.fill("input[name='q']", "web automation")   # 填写搜索框
    page.click("button[type='submit']")               # 点击提交
    page.wait_for_selector(".result")                 # 等待结果加载
    print(title, [el.inner_text() for el in page.query_selector_all(".result")[:3]])
    browser.close()
```

`requests` 适合接口与静态页、`BeautifulSoup`/`lxml` 适合解析、Playwright 适合需要执行 JS 与模拟交互的场景，三者按页面是否需要渲染来选型。

## 小结

能走接口就别开浏览器；必须开浏览器时，用显式等待代替固定 sleep、用稳定属性代替脆弱的层级路径。选择器写得越依赖结构，页面一改就越容易失效。

## 易错点

- DOM/XPath 写死：页面一改选择器全挂，优先用稳定的 `data-*` 属性或语义选择器。
- 不等待动态加载：元素还没渲染就操作，随机失败；必须用显式等待。
- 忽视反爬/风控：频率过高被封 IP，需限速与 UA 轮换。
- 把登录态/验证码硬编码：会话一过期脚本就失效。
- 并发爬取不节流：既打垮目标站也让自己被 ban。

## 练习

动手检验一下自己：

1. 如何判断目标数据是服务端渲染还是前端 JS 动态生成？给出两种验证方法。
2. XPath 相比 CSS 选择器多了哪些能力？各写一个只能用 XPath 表达的定位。
3. 固定 `sleep(3)` 等待元素加载有什么问题？应该改用什么机制？
