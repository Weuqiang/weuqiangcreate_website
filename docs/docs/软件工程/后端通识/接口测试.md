...
sidebar_position: 2
title: 接口测试
description: "接口测试实战：requests / aiohttp / httpx 三大客户端选型，同步与异步请求模式对比，HTTP/2 多路复用下的性能差异"
---
...
- 掌握 requests / aiohttp / httpx 三个客户端库的定位与选型依据
- 分清同步与异步请求模式，理解并发对吞吐量的影响
- 了解 HTTP/2 多路复用相对 HTTP/1.1 的差异及其对测试策略的影响
...
- [接口开发](../接口开发)（先会写接口再测接口）
- [Python · 30异步编程](../../../编程语言/Python/30异步编程)

## 核心概念

接口测试的瓶颈几乎总在网络等待而非计算，因此并发模型的选择直接决定测试效率。同步库（requests）一次一个请求，串行等待；异步库（aiohttp/httpx）用事件循环在等待期间发出更多请求，吞吐量可以高一两个数量级。httpx 的特殊价值在于同时支持同步/异步 API 和 HTTP/2，能在同一套代码里对比不同模式。

本章主线：
- requests：最通用的同步客户端
- aiohttp 与 httpx：异步与 HTTP/2 支持
- 同步请求模式 A / B 对比
- 异步请求模式 A / B 与 H2 下的表现
...
接口测试的核心是「对约定好的输入输出做断言」。下面用 `pytest` + `requests` 给一个 REST 接口写最小回归用例：

```python
# 用 pytest + requests 对 REST 接口做断言式测试
import requests
...
def test_create_user():
    r = requests.post(f"{BASE}/users", json={"name": "wei"}, timeout=5)
    assert r.status_code in (200, 201)
    assert r.json()["name"] == "wei"
```

除了功能断言，接口测试还应覆盖状态码、超时、错误码与幂等性；配合上文的 `locust` 再做一轮压测，就能同时保证「正确」与「扛压」。

## 小结

选客户端看三点：要不要异步、要不要 HTTP/2、要不要与现有同步代码兼容。测吞吐时务必区分「服务端慢」和「客户端串行」这两种完全不同的瓶颈。

## 易错点

- 只测 happy path：异常输入、边界、超时、并发不覆盖，线上才爆。
- 用例间相互污染：依赖固定环境或共享数据，本地过 CI 挂，根因是没隔离。
- 拿生产库当测试库：脏数据事小，误改真实订单事大。
- 断言只看状态码不看 body：返回 200 但业务字段为空，逻辑错误被漏掉。
- 忽视鉴权/权限用例：越权漏洞测不出来，等于没测安全。

## 练习

动手检验一下自己：

1. 同样发 100 个请求，requests 串行与 aiohttp 并发的耗时差多少？瓶颈分别在哪？
2. HTTP/2 的多路复用解决了 HTTP/1.1 的什么问题？对连接数有何影响？
3. 为什么复用 Session/Client 对象比每次新建更快？涉及哪些开销？
