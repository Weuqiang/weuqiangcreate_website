...
sidebar_position: 99
title: 练习
description: "Python 编程综合练习题集：进度条、后缀表达式、密码、质数分解、排序、单例模式等数十道从基础到进阶的动手练习"
---
...
- 把前面各章的语法点串联起来，在完整的小问题中检验掌握程度
- 养成「先独立作答、再看题解」的练习习惯，暴露真实的知识盲区
- 积累常见编程套路：字符串格式化、栈的应用、数论、排序与设计模式

## 前置知识

在阅读下面的内容前，建议先掌握：

- 建议已读完 [0基础语法](../0基础语法) 到 [39模块](../39模块) 各章
- 算法思路补充：[数据结构与算法](../../../计算机科学基础/数据结构与算法/)

## 核心概念

本页是综合题集，每题分「描述」与「题解」两部分。正确的用法是先只读描述、自己动手写一遍、跑通之后再对照题解——直接看答案的学习效率远低于先卡住再看。题目按难度递进，覆盖字符串处理、数据结构应用、数学计算与设计模式四类典型场景。
...
语法看懂不等于会写。这一页的价值在于把碎片语法压进真实问题里，做完之后再回看前面各章，很多细节会突然变得清晰。

## 易错点

- 别直接看答案：先自己写、再对照题解，才能真正暴露知识盲区。
- 边界条件常被忽略：空输入、单元素、超大数都是隐藏用例，别只测 happy path。
- 命名可读性优先于简短：`tmp2` 不如 `sorted_indices` 清楚。
- 写完要跑一下：语法对不等于逻辑对，用几个例子验证。
- 善用 `assert` 自测：把预期写进断言，便于回归与调试。

## 延伸阅读

- 上一篇：[Python · 39模块](../39模块)
- 返回 [Python 语言总览](../)
