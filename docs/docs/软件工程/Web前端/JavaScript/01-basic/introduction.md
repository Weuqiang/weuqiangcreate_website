...
- 说清 JavaScript 的定位（浏览器脚本 / 全栈语言）
- 理解 JS 与 HTML/CSS 的协作关系
- 知道 JS 的主要运行环境（浏览器、Node.js）

## 前置知识

在阅读下面的内容前，建议先掌握：

- 先读 [HTML 入门](/docs/软件工程/Web前端/HTML/intro)

## 核心概念

JavaScript 是网页的“行为层”语言，最初为浏览器交互而生，现通过 Node.js 也能写服务端，成为真正的全栈语言。它是**动态、弱类型、基于原型、事件驱动**的语言：变量类型运行时确定、不同类型可隐式转换、对象通过原型链继承、靠事件循环处理异步。

本章主线：
- 什么是 JavaScript 语言
- 为什么学习 JavaScript（生态与就业）
- 实验环境与运行方式
...
JS 是动态弱类型、基于原型、事件驱动的语言，从浏览器脚本发展为全栈语言；与 HTML/CSS 共同构成前端三件套。

## 易错点

- **别依赖分号省略**：JS 的“自动分号插入”（ASI）会在多数换行处补分号，但 `return` 后换行、`(` / `[` / `` ` `` 开头的行会被错误合并，造成静默 bug；需要换行续写时加前缀保护。
- **别用 `var`**：`var` 是函数作用域且会“变量提升”（hoisting），在循环或回调里常拿到意外值；一律用 `let` / `const`。
- **`==` 会做隐式类型转换**：`0 == ''`、`'1' == 1` 都为 `true`，比较一律用 `===`，避免被类型转换坑到。
- **数组/对象是引用比较**：`[1] === [1]`、`{}` === `{}` 都是 `false`，相等判断要看内容或引用是否同一对象。
- **`this` 的取值取决于调用方式**：普通函数、方法、箭头函数、回调里的 `this` 规则不同，是 JS 最容易踩的坑之一（详见后续“this”章节）。

## 练习

动手检验一下自己：

1. 在浏览器控制台执行一句 JS，确认运行环境。
2. 用一句话区分 JS 与 Java（提示：毫无关系，仅名字像）。
3. 列出 JS 可以做、而纯 HTML 做不到的三件事。
