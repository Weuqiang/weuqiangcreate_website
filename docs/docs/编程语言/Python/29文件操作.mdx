...
- 掌握 `open` 的模式组合（r/w/a/x/b/+）与编码参数，避免中文乱码
- 理解 `with` 上下文管理器如何保证文件句柄一定被释放
- 了解 `memoryview` 的零拷贝切片在处理大二进制数据时的价值

## 前置知识

在阅读下面的内容前，建议先掌握：

- [Python · 2字符串](../2字符串)（文本模式读写涉及编码解码）
- 对照阅读：[C 语言 · 文件输入输出](../../C/11文件输入输出)、[操作系统 · 文件系统](../../../计算机科学基础/操作系统/文件系统)

## 核心概念

文件对象是操作系统文件描述符的 Python 封装。文本模式（`r`）会在读写时自动做编码转换和换行符转换，二进制模式（`rb`）则原样传递字节——处理图片、压缩包时必须用二进制模式。`with` 语句依赖上下文管理协议（`__enter__` / `__exit__`），即使块内抛出异常也保证关闭文件，这是唯一推荐的写法。
...
永远用 `with` 打开文件、永远显式指定 `encoding`、二进制数据永远用 `b` 模式。这三条能避免文件操作里绝大多数线上事故。

## 练习

动手检验一下自己：

1. 不用 `with` 而手动 `close()`，如果中间抛出异常会发生什么？
2. `'w'` 与 `'a'` 模式的区别是什么？`'x'` 模式在文件已存在时如何表现？
3. 读取一个 GBK 编码的文本文件却不指定 `encoding`，在 Windows 与 Linux 上结果会一样吗？为什么？

## 易错点

- 用完要关：不 `close()` 数据可能没刷盘；用 `with open(...) as f` 自动关闭最稳妥。
- 模式别混：读模式 `r` 打开不存在文件报 `FileNotFoundError`；写模式 `w` 会清空原文件。
- 编码问题：`open` 默认依赖平台编码（Windows 常为 gbk），读 UTF-8 文件需显式 `encoding='utf-8'`。
- 路径拼接用 `os.path.join` / `Path`：手工拼 `/` 在 Windows 上会出错。
- 读大文件别一次性 `read()`：逐行迭代或分块读取，避免内存暴涨。

## 延伸阅读

- 上一篇：[Python · 28装饰器与作用域](../28装饰器与作用域)
- 下一篇：[Python · 30异步编程](../30异步编程)
- 返回 [Python 语言总览](../)
