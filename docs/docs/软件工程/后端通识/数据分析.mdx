...
- 掌握 NumPy 的 ndarray、广播机制与向量化运算
- 会用 Pandas 完成数据读取、清洗、分组聚合与合并
- 能选择合适的可视化形式呈现分析结论
...
- [Python · 3序列](../../../编程语言/Python/3序列)、[Python · 10函数](../../../编程语言/Python/10函数)
- 数学支撑：[线性代数](../../../数学基础/线性代数/)、[概率与统计](../../../数学基础/概率与统计/)

## 核心概念

NumPy 的核心贡献是把「循环处理每个元素」变成「对整个数组做一次运算」——底层用 C 实现的连续内存 + 向量化指令，让 Python 在数值计算上重新获得可用的性能。Pandas 在 ndarray 之上加了带标签的行列索引，把数组变成了「可以按名字操作的表格」。二者的共同心法是：**能用向量化就不要写 for 循环**。

本章主线：
- NumPy：ndarray、切片、广播与常用函数
- Pandas：Series 与 DataFrame、读写、清洗
- 分组聚合、合并与透视
- 数据可视化
...
把上文的读取、分组、透视串成一个最小闭环：读入 → 概览 → 聚合 → 出图，拿到第一眼结论。

```python
# 一份最小数据分析闭环：读入 -> 概览 -> 分组聚合 -> 可视化
import pandas as pd
import matplotlib.pyplot as plt

df = pd.read_csv("sales.csv")                 # 读入数据
print(df.describe())                          # 数值列统计概览
print(df.groupby("city")["sales"].sum())      # 按城市汇总销售额

# 画一张趋势图
df["date"] = pd.to_datetime(df["date"])
df.plot(x="date", y="sales", kind="line", title="Sales Trend")
plt.savefig("sales_trend.png")
```

`describe()` 先快速发现缺失值与异常分布，`groupby` 做维度下钻，最后用图把趋势讲给别人听——这三步几乎出现在每一次分析的起点。

## 小结

NumPy 管数值、Pandas 管表格、可视化管表达。写分析代码时若出现显式 for 循环遍历行，通常意味着有更快的向量化写法。

## 易错点

- 用均值掩盖分布：偏态或双峰数据均值毫无意义，要看分位数与直方图。
- 相关当因果：两列同涨就下结论，忽略混淆变量。
- 拿全量当样本做推断却不随机：结论无法外推到总体。
- 时间字段时区错：跨时区聚合全乱，先统一到 UTC 再算。
- p 值当效应大小：显著但效应微小，业务上没意义。

## 练习

动手检验一下自己：

1. 广播机制在什么条件下成立？举一个形状不兼容而报错的例子。
2. 用 Pandas 对一份销售数据按地区分组求销售额均值，并筛出高于总体均值的地区。
3. 同样对百万行数据做逐行计算，`iterrows` 与向量化运算的耗时差多少？为什么？
