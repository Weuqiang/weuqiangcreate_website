...
- 零基础友好；想对比其他语言可看 [C 0 基础语法](/docs/编程语言/C/0基础语法)
- 或 [Python 0 基础语法](/docs/编程语言/Python/0基础语法)

## 核心概念

HTML（超文本标记语言）负责网页的**结构**：它用“标签”描述内容的语义，浏览器再把结构渲染成可见页面。它与 CSS（样式层）、JavaScript（行为层）分工协作——HTML 只管“是什么”，不管“长什么样、能交互”。一个 HTML 文档由若干“元素”组成，元素由开始标签、内容和结束标签构成，标签可带“属性”提供额外信息。
...
HTML 是网页的结构层，用标签表达内容语义；标签+属性构成元素。它只描述“是什么”，样式与交互交给 CSS 和 JavaScript。

## 练习

动手检验一下自己：

1. 写一个包含标题、一段文字和一个链接的最小 HTML 页面。
2. 指出 `<a href="x">文字</a>` 中哪部分是标签、属性、内容。
3. 用浏览器打开你的页面，查看源代码确认结构。
