...
sidebar_position: 4
title: Terminal
description: "命令行与终端实战：WSL 环境搭建、目录导航与文件管理、快捷键、管道重定向等进阶技巧，以及用 Makefile 固化常用任务"
---
...
- 理解终端、Shell、命令行三者的关系，会在 Windows 上用 WSL 获得 Linux 环境
- 掌握文件管理、管道与重定向、通配符等日常命令行技能与常用快捷键
- 会写基础的 Makefile 把重复操作固化成可复用的命令
...
- 了解文件与目录的基本概念；无需编程基础
- 延伸使用：[Git](../Git)、[Docker](../Docker) 均以命令行为主要界面

## 核心概念

终端是「输入文本、得到文本」的界面，Shell 是解释这些文本的程序。命令行之所以在图形界面普及几十年后依然不可替代，靠的是**可组合性**：每个命令只做一件事，通过管道把上一个命令的输出接到下一个命令的输入，就能临时拼装出任意复杂的处理流水线——而且这条流水线可以被复制、保存、写进脚本自动执行。
...
终端的强大之处在于「管道」：把一个命令的输出喂给下一个。下面用经典的日志分析组合统计访问最多的 IP，再用 `du` 找出大文件：

```bash
# 统计日志中出现最多的前 5 个 IP（经典管道组合）
cat access.log \
  | awk '{print $1}' \        # 取第一列（IP）
  | sort \                    # 排序以便去重计数
  | uniq -c \                 # 统计每个 IP 出现次数
...
`|` 把左侧 stdout 接到右侧 stdin，`awk`/`sort`/`uniq` 各司其职，组合出的能力远超单个命令，这正是终端效率的来源。

## 小结

命令行的核心竞争力是可组合与可复用：小工具 + 管道 + 脚本。先把十来个高频命令和快捷键练成肌肉记忆，再用 Makefile 收编重复劳动。

## 易错点

- 用 `rm -rf` 带未判空变量：`rm -rf $DIR/` 当 DIR 为空变成删根，灾难性。
- 在错误目录执行破坏性命令：动手前先 `pwd`/确认当前位置。
- 用 `sudo` 跑不明脚本：把不可信内容直接提权执行。
- 管道吞掉错误：`grep` 失败被 `&&` 当成成功，掩盖问题。
- 把别名/函数写进非 rc 文件：新 shell 不生效，排查半天。

## 练习

动手检验一下自己：

1. 用一行命令统计当前目录下所有 .py 文件的总行数，说明每个管道段的作用。
2. 解释标准输出与标准错误的区别，写出把二者分别重定向到不同文件的命令。
3. 为一个项目写 Makefile，包含 install、test、clean 三个目标。
