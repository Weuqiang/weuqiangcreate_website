...
1. **后端服务**：负责调用 AI API 生成摘要并存储
2. **前端组件**：负责获取摘要并展示逐字显示效果
...
class Jsonsummary:
    def __init__(self):
        ...
        self.url = "https://weuqiang.github.io/"
        self.pages = []
...
def fetch_blog():
    content = feedparser.parse("https://weuqiang.github.io/blog/rss.xml")["entries"]
...
把`BAIDU_API_KEY`和`BAIDU_SECRET_KEY`传入 git action 的环境中的示例：
...
完整代码参考我的 [GitHub 仓库](https://github.com/Weuqiang/weuqiangcreate_website)
...
const JsonReader = ({
  ...
  // 替换url与/
  const path = fieldToMatch.replace(/https:\/\/weuqiang.github.io\//, "").replace(/\//g, "_");
  const url = `https://weuqiang.github.io/Weuqiang/summary/${path}.json`;
