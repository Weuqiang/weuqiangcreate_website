...
sidebar_position: 1
title: 接口开发
description: "API 的请求、响应与端点构成，OpenAPI、MCP 等接口标准对比与框架选型，FastAPI 基础用法与二进制传输、MCP 服务开发及 Django 接口实践"
---
...
- 理解 REST 风格的资源建模、HTTP 方法语义与状态码约定
- 会用 FastAPI 快速搭建带自动文档与数据校验的接口服务
- 了解 Django 的全栈定位与 MCP 协议在 AI 工具调用中的角色

## 前置知识

在阅读下面的内容前，建议先掌握：

- [Python · 10函数](../../../编程语言/Python/10函数)、[Python · 30异步编程](../../../编程语言/Python/30异步编程)
- 协议基础：[HTTP 与网络安全](../../../计算机科学基础/计算机网络/HTTP与网络安全)

## 核心概念

接口的本质是「跨进程的函数调用约定」。REST 把这套约定映射到 HTTP 语义上：URL 定位资源、方法表达动作、状态码表达结果、请求体与响应体承载数据。框架的价值在于把序列化、校验、路由、文档这些重复劳动自动化——FastAPI 用类型注解同时完成校验和文档生成，这是 Python 类型提示落地最成功的场景之一。

本章主线：
- API 概述：REST 风格与 HTTP 语义
- FastAPI：路由、依赖注入、数据校验与自动文档
- MCP：面向 AI 工具调用的接口协议
- Django：全栈框架与 ORM
...
参考地址：[FastMCP GitHub 仓库](https://github.com/jlowin/fastmcp)
...
更多信息请参考 [MCP 官方文档](https://py.sdk.modelcontextprotocol.io/) 和 [PyPI 包页面](https://pypi.org/project/mcp/)。
...
同样的 REST 资源，用 FastAPI 实现会比 Django 更轻量直观，且自带交互式文档，适合快速验证接口设计：

```python
# 用 FastAPI 实现同样的 REST 资源（相比 Django 更轻量直观）
from fastapi import FastAPI
from pydantic import BaseModel
...
def put_item(item_id: int, item: Item):
    db[item_id] = item
    return {"method": "POST", "id": item_id, "data": item}

# 启动：uvicorn main:app --reload  然后访问 /docs 自动获得交互式 API 文档
```

`@app.get` / `@app.post` 直接绑定路径与函数，Pydantic 负责请求体校验，省去了 Django 里 serializer 与 view 的样板代码；生产里再按需换回 Django 也不影响接口契约。

## 小结

接口设计先想清楚「资源是什么、动作是什么、失败怎么表达」，再选框架。FastAPI 适合数据驱动的 API 服务，Django 适合需要后台与 ORM 的完整应用。

## 易错点

- 忽视幂等：网络重发/重试导致重复下单或重复扣款；写接口必须带幂等键（Idempotency-Key）。
- 把内部错误细节（堆栈、SQL）直接返前端：既泄露实现又便于攻击，应统一错误码与文案。
- 用 HTTP 状态码当业务码：200 裹着 `{code:500}`，客户端与健康检查都误判成功。
- 接口不做版本控制（/v1）：后期不兼容改动直接 break 老客户端。
- 入参不校验/不限定类型：这是注入与脏数据的最大入口。

## 练习

动手检验一下自己：

1. PUT 与 PATCH 的语义差异是什么？幂等性要求分别是怎样的？
2. 用 FastAPI 写一个带路径参数、查询参数和请求体校验的接口，并说明自动文档从何而来。
3. 接口返回 200 但业务失败，与直接返回 400，两种设计各有什么利弊？
...
- 跨主题关联：[HTTP 与网络安全](../../../计算机科学基础/计算机网络/HTTP与网络安全)（RESTful 语义、状态码、鉴权与 CORS 的协议基础）
