...
sidebar_position: 3
title: 序列
description: "Python 序列类型系统详解：列表、字典、集合、元组的常用方法、增删改查与推导式，以及 len、slice、sorted、reversed、filter 等内置函数实用用法"
---
...
- 掌握列表、字典、集合、元组四大容器的特性、方法族与适用场景
- 理解可变（list/dict/set）与不可变（tuple/str）的分界，及其对哈希与函数默认参数的影响
- 会按操作复杂度选择容器：查找用字典/集合、顺序访问用列表、去重用集合
...
- [Python · 2字符串](../2字符串)
- 底层结构对照：[数组与链表](../../../计算机科学基础/数据结构与算法/数组与链表)、[哈希表](../../../计算机科学基础/数据结构与算法/哈希表)

## 核心概念

四大容器可以用两个维度切分：**有序 vs 无序**、**可变 vs 不可变**。列表是动态数组（尾部追加均摊 O(1)，中间插入 O(n)）；字典和集合是哈希表（平均 O(1) 查找，要求键可哈希，因而键必须不可变）；元组是不可变列表，因此可以做字典的键。选错容器不会报错，但会让性能相差几个数量级。

本章主线：
- 列表：动态数组、切片、方法分类
- 字典：哈希映射、键的可哈希要求、视图对象
- 集合：去重与集合运算
- 元组：不可变序列与解包
...
列表管顺序、字典管映射、集合管唯一、元组管固定。判断依据是「是否需要按键查找」和「是否需要修改」，选对容器就同时选对了复杂度。

## 练习

动手检验一下自己：

1. 为什么列表不能做字典的键，而元组可以？可哈希的必要条件是什么？
2. `def f(x, lst=[])` 这个默认参数为什么危险？给出正确写法。
3. 在 100 万条数据里判断某元素是否存在，用 `list` 和用 `set` 的时间复杂度分别是多少？

## 易错点

- 列表切片产生副本：`lst[1:3]` 是新列表，修改它不影响原列表，但嵌套列表只做浅拷贝。
- `list.sort()` 原地排序并返回 `None`：若写成 `sorted_lst = lst.sort()` 会得到 `None`。
- 元组「不可变」仅限顶层：元组内嵌的列表仍可修改，别误以为整个结构被冻结。
- 可变对象不能作 dict 的 key、也不能放进 set：它们必须可哈希（hashable）。
- `in` 在 list 是 O(n)、在 set/dict 是 O(1)：大数据量用错结构会严重拖慢。

## 延伸阅读

- 上一篇：[Python · 2字符串](../2字符串)
- 下一篇：[Python · 5控制流](../5控制流)
- 返回 [Python 语言总览](../)
