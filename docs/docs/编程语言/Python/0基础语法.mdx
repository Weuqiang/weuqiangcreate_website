...
sidebar_position: 0
title: 基础语法
description: "Python 基础语法完整入门：Python 之禅、缩进与注释、关键字、内置数据类型、异常、函数、常量、运算符与标准库概览"
---
:::info
...
- 掌握 Python 以缩进（而非花括号）划分代码块的语法基石，以及注释与命名约定
- 熟悉 35 个关键字、基础数据类型、四大容器类型与迭代器/生成器的全貌
- 认识内置异常的继承体系，会用常用内置函数与运算符优先级表

## 前置知识

在阅读下面的内容前，建议先掌握：

- 无需 Python 前置，但需要基本的计算机操作能力；完全零基础可先浏览 [计算机科学导论](../../../计算机科学基础/计算机科学导论/)

## 核心概念

Python 的设计哲学是「一切皆对象、可读性至上」。缩进不是排版偏好而是语法本身——同一缩进层级的语句属于同一代码块，这从语言层面强制了统一的视觉结构。所有值（数字、函数、类、模块）都是对象，都有类型、身份和引用计数，这解释了后面章节里的赋值语义、参数传递与内存行为。
...
本章勾勒了 Python 的语法地图：缩进定义结构、关键字划定边界、内置类型与内置函数构成日常工具箱、异常体系兜底错误处理。后面每一章都是在这张地图上的深挖。

## 练习

动手检验一下自己：

1. 为什么 Python 用缩进而不是花括号？这个设计对代码审查和多人协作有什么影响？
2. `is` 和 `==` 的判断依据分别是什么？在什么场景下二者结果会不一致？
3. 写出 `BaseException`、`Exception`、`ValueError` 三者的继承关系，并说明为什么捕获异常时不推荐直接写 `except BaseException`。

## 易错点

- 缩进必须一致：混用 Tab 与空格会在运行时报 `TabError`；统一用 4 个空格最省心。
- 复合语句末尾的冒号：`if` / `for` / `while` / `def` / `class` 后面漏写 `:` 会直接语法错误。
- 整数除法的行为：`3 / 2` 在 Python 3 里得到 `1.5`（float），要整除用 `//`；这与 Python 2 不同。
- 变量未定义就使用：最常见的 `NameError`，注意变量要先赋值再引用。
- `print` 是函数不是语句：写成 `print x` 会报错，必须加括号 `print(x)`。
