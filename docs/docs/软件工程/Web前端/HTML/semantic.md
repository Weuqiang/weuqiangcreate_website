...
- 区分语义标签与纯表现标签（如 `<div>` vs `<header>`）
- 用 `<header>/<nav>/<main>/<article>/<footer>` 组织页面骨架
- 说清语义化对可访问性与 SEO 的好处
...
语义化指“用含义正确的标签表达结构”：`<header>` 表示页眉、`<nav>` 表示导航、`<main>` 表示主体、`<article>` 表示独立内容、`<footer>` 表示页脚。相比万能的 `<div>`，语义标签让浏览器、屏幕阅读器、搜索引擎更准确地理解页面，既提升可访问性，也利于 SEO。
...
语义化是用 `<header>/<nav>/<main>/<article>/<footer>` 等含义明确的标签组织页面，让机器与人都能读懂结构，可访问性与 SEO 双赢。

## 练习

动手检验一下自己：

1. 把一段用多个 `<div>` 堆出的页面改用语义标签重构。
2. 解释为什么导航用 `<nav>` 比 `<div class="nav">` 更好。
3. 用屏幕阅读器或无障碍检查工具验证你的语义结构。
