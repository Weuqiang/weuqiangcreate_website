...
- 进阶组织见 [语义化标签](/docs/软件工程/Web前端/HTML/semantic)

## 核心概念

HTML 提供丰富的文本语义标签：`<p>` 表示段落、`<div>` 是无语义块容器、`<span>` 是无语义行内容器；强调用 `<strong>`（重要）/`<em>`（语气），引用用 `<blockquote>`（块）/`<q>`（行内），代码用 `<code>`、缩略用 `<abbr>` 等。优先选语义标签而非“`<div>`+class”，可读性更好。
...
文本标签分块级（`<div>/<p>`）与行内（`<span>` 及语义标签）；强调用 `<strong>/<em>`、引用用 `<blockquote>/<q>`、代码用 `<code>`，语义优先于样式。

## 练习

动手检验一下自己：

1. 把一段“重要提示”从 `<div class="imp">` 改为语义标签。
2. 说明 `<strong>` 与 `<b>` 的语义差异。
3. 用 `<blockquote>` 与 `<q>` 分别表示一个长引用与短引用。
