...
- [Number 类型](/docs/软件工程/Web前端/JavaScript/types/number)

## 核心概念

Math 是内置命名空间，提供 PI、E 等常量与 abs/ceil/floor/round/random/max/min 等方法。方法是静态的，不可实例化。
...
Math 为静态工具集；random 返回 0 到 1 区间，需缩放得到目标区间。

## 练习

动手检验一下自己：

1. 用 Math.random 生成 1 到 10 的整数。
2. 用 max/min 配合展开运算求数组极值。
