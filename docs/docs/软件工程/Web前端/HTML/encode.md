...
- 理解字符→码点→字节的编码链路
- 说清 ASCII、Unicode、UTF-8 的关系
- 会用 HTML 字符实体表示保留字符，理解乱码成因
...
- 二进制背景见 [数字逻辑与冯诺依曼](/docs/计算机科学基础/组成原理/数字逻辑与冯诺依曼)

## 核心概念

字符编码分两步：先把字符映射为“码点”（Unicode 统一编号），再把码点编码为字节（UTF-8 变长）。ASCII 只覆盖英文字符（1 字节），Unicode 覆盖全球文字，UTF-8 是其最常用编码：英文 1 字节、中文通常 3 字节。HTML 中用“字符实体”（如 `&lt;`、`&amp;`）表示 `<`、`&` 等保留字符；当文件编码与浏览器解码方式不一致时就会出现乱码。
...
字符先成码点（Unicode）再成字节（UTF-8）；声明一致的编码即可避免乱码，HTML 用字符实体表示 `<`、`&` 等保留字。

## 练习

动手检验一下自己：

1. 解释为什么中文在 GBK 文件用 UTF-8 打开会乱码。
2. 写出 `<` 和 `&` 的 HTML 实体写法。
3. 用 `meta charset` 声明 UTF-8，验证中英文混合页面不乱码。
