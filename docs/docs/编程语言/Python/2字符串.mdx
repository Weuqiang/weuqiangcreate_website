...
- 理解字符串的不可变性及其对拼接性能的影响（应改用 `join`）
- 掌握 f-string、`format`、`%` 三代格式化与常用字符串方法族
- 分清 `str` 与 `bytes`、编码与解码，避免中文乱码
...
- [Python · 1变量](../1变量)
- 底层对照：[C 语言 · 字符串和字符串函数](../../C/9字符串和字符串函数)（以 `\0` 结尾的字符数组 vs 不可变对象）

## 核心概念

Python 的字符串是不可变的 Unicode 码点序列。不可变意味着每次「修改」都在创建新对象——循环里用 `+=` 拼接 n 个字符串是 O(n²)，用 `join` 才是 O(n)。`str` 是抽象的字符序列，`bytes` 是具体的字节序列，二者之间靠编码（encode）和解码（decode）转换，编码选错就是乱码的根源。
...
字符串不可变、按 Unicode 码点索引、格式化优先用 f-string、批量拼接必须用 `join`。与外界交换数据时始终显式指定编码。

## 练习

动手检验一下自己：

1. 在循环中用 `s += x` 拼接 10 万个字符串，与用 `''.join(list)` 相比复杂度差多少？为什么？
2. `'abc'.replace('a', 'z')` 会修改原字符串吗？函数返回的是什么？
3. 把含中文的字符串编码成 UTF-8 与 GBK，字节长度分别是多少？解码时用错编码会发生什么？

## 易错点

- 字符串不可变：`s[0] = 'x'` 会报错；要改只能重新赋值或切片拼接。
- `+` 拼接大量字符串性能差：循环里用 `+=` 是 O(n²)，应收集到 list 再 `''.join()`。
- f-string 的大括号：`{` 与 `}` 自身要用 `{{` / `}}` 转义，否则被当作表达式边界。
- str 与 bytes 不能直接相加：`'a' + b'bytes'` 直接 `TypeError`，需显式 `.encode()` / `.decode()`。
- 索引越界没有「负数万能」：`s[-10]` 对长度为 3 的字符串仍然 `IndexError`。
