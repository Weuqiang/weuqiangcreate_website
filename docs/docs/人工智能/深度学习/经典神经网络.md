...
sidebar_position: 5
title: 经典神经网络
description: "从感知机到多层网络，讲清激活函数、反向传播与梯度消失，奠定深度学习直觉。"
---
...
- 理解感知机到多层网络，以及为何必须有非线性激活
- 掌握反向传播「前向算输出、回传算梯度」的基本套路
- 认识梯度消失/爆炸与初始化、归一化的关系

## 前置知识

在阅读下面的内容前，建议先掌握：

- 先读 [机器学习/回归算法](../../机器学习/回归算法) 看线性映射的连续版
- 基础见 [矩阵与线性方程组](/docs/数学基础/线性代数/矩阵与线性方程组) 与 [梯度与方向导数](/docs/数学基础/微积分/梯度与方向导数)

## 核心概念

经典神经网络 = 多层「加权求和+非线性激活」的堆叠。没有非线性激活，堆多少层都等价于一个线性变换，所以激活函数（ReLU 等）才是让网络「能拟合任意曲线」的关键。训练靠反向传播：前向算输出、算损失，再把梯度沿网络回传，逐层更新权重。它把「怎么学」从手工特征变成端到端自动学表示。
...
上文用 PyTorch 训练了多层网络，这里退到最底层：用 NumPy 从零实现单层感知机，靠梯度下降学会逻辑「与（AND）」，体会反向传播的最小形态。

```python
# 从零实现单层感知机，用梯度下降学习逻辑 AND
import numpy as np

X = np.array([[0,0],[0,1],[1,0],[1,1]])
y = np.array([0, 0, 0, 1])          # AND 标签
...
print("预测:", [1 if (xi @ w + b) > 0 else 0 for xi in X])   # -> [0, 0, 0, 1]
```

感知机只能学线性可分问题（AND 可分时能收敛，XOR 则永远学不会），这正是后续引入多层网络与非线性激活函数的根本原因。

## 小结

神经网络用非线性激活+反向传播把表示学习自动化。激活选错或初始化不当会梯度消失，现代实践靠 ReLU 族、合理初始化和归一化化解。

## 易错点

- 梯度消失/爆炸：深层网络反向传播时梯度连乘，Sigmoid/Tanh 易让梯度趋零或爆炸；用 ReLU + 合理初始化（He/Xavier）+ 批归一化缓解。
- 初始化不当直接训不动：全零初始化让神经元对称更新、学不到东西，必须随机初始化。
- 学习率是最关键的超参：太大不收敛、太小慢到没边，先用小学习率跑通再调。
- 过拟合小数据：网络容量远大于数据量时会死记，需 dropout / 正则 / 早停。

## 练习

动手检验一下自己：

1. 去掉所有激活函数，深层网络为什么退化成线性模型？
2. 反向传播和「从输出往回算梯度」是一回事吗？简述直觉。
3. sigmoid 为何容易梯度消失，ReLU 怎么缓解？
