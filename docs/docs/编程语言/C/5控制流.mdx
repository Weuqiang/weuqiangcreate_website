...
- 掌握 `if/else`、`switch` 选择结构与 `for`/`while`/`do-while` 循环
- 理解 `break`/`continue`/`goto` 对流程的控制
- 能根据问题选择恰当的控制结构并避免死循环
...
程序默认顺序执行；控制流语句通过「条件真假」决定走哪条分支、重复哪段代码。C 的真假性很简单——非 0 即真、0 即假，这是所有条件判断的底层约定。
...
本章让程序「会做判断、会重复」：选择结构按条件分支，循环结构按计数或条件重复，跳转语句精细控制流程。牢牢记住 C 用「非 0 即真」统一了所有条件语义。

## 练习

动手检验一下自己：

1. `switch` 的 `case` 后面如果不写 `break` 会发生什么？举例说明。
2. 用 `for` 与 `while` 分别写出一个累加 1~100 的循环，并指出二者语义差异。
3. 什么情况下 `do-while` 与 `while` 的行为不同？给一个必要性例子。

## 易错点

- `case` 分支漏写 `break`：执行会「贯穿」到下一个分支继续跑，出现一次输入触发多段逻辑的怪现象；确实需要贯穿时应加注释显式说明。
- 在 `if`、`for`、`while` 的圆括号后多打一个分号：`for (int i = 0; i < n; i++);` 中的分号是空循环体，真正的代码块只会在循环结束后执行一次。
- 循环变量用无符号类型倒着减到 0：`for (unsigned i = n - 1; i >= 0; i--)` 的条件恒成立（无符号数永远不小于 0），直接死循环；倒序遍历应改用有符号类型或换成 `while` 判断。
- 单行分支不写花括号：日后补一行代码时只有第一行受条件控制，缩进看着完全正确，逻辑却已经错了。
- `break` 只能跳出**一层**循环：嵌套循环里想一次跳到最外面，需要借助标志变量或 `goto`，误以为 `break` 能跳出全部是常见错误。
- `switch` 的判断表达式必须是整型或枚举，不能是 `float`、`double` 或字符串；`case` 标签也必须是编译期常量，写成变量会直接编译报错。

## 易错点

- **`if` 后误加分号**：`if (x > 0); { ... }` 的分号使它变成"空语句 + 永远执行的块"，逻辑全错。
- **`==` 写成 `=`**：赋值表达式有值，会让条件恒真；推荐用 `if (5 == x)` 让漏写直接编译报错。
- **`switch` 漏 `break`**：忘记 `break` 会发生 fall-through，继续执行下一个 `case`，常是隐蔽 bug。
- **循环边界 off-by-one**：`for (i = 0; i <= n; ...)` 比预期多一轮，注意是 `<` 还是 `<=`。
- **浮点做计数器**：用 `double` 当循环计数器会因累积误差少跑或多跑一轮，整数更安全。
