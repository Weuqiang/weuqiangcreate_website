...
- 理解 JS 为何是单线程却“不卡”
- 说清事件循环、宏任务与微任务
- 知道异步编程的演进：回调 → Promise → async/await

## 前置知识

在阅读下面的内容前，建议先掌握：

- 先读 [JavaScript 简介](/docs/软件工程/Web前端/JavaScript/basic/introduction)
- 并发背景见 [进程、线程与调度](/docs/计算机科学基础/操作系统/进程线程与调度)

## 核心概念

JS 在浏览器中是**单线程**执行的，但靠“事件循环”处理异步：耗时操作（网络、定时）不阻塞主线程，完成后把回调放进任务队列，待主线程空闲时依次执行。任务分宏任务（setTimeout 等）与微任务（Promise.then 等），微任务优先。演进路径是 回调 → Promise → async/await，逐步摆脱“回调地狱”。

本章主线：
- 单线程模型与为何不阻塞
- 同步任务 vs 异步任务
- 任务队列与事件循环
- 异步模式演进：回调 / Promise / async-await
...
JS 单线程靠事件循环调度异步：宏任务与微任务排队执行、微任务优先；异步写法从回调走向 Promise 与 async/await，可读性大幅提升。

## 练习

动手检验一下自己：

1. 画一个“主线程 → 任务队列 → 事件循环”的简要流程图。
2. 预测一段 `setTimeout` 与 `Promise.then` 混合代码的打印顺序。
3. 把一个回调嵌套改写为 async/await 形式。

## 延伸阅读

- 理论对照：[进程、线程与调度](../../../../../计算机科学基础/操作系统/进程线程与调度)（JS 单线程 + 事件循环是"协作式并发"，对照 OS 抢占式多线程调度）
- 跨语言并发：[Go goroutine 与 channel](../../../../../编程语言/Go/并发模型/goroutine与channel) · [Python 异步编程](../../../../../编程语言/Python/30异步编程)
