...
- 理解 Python 整数的任意精度实现与浮点数的 IEEE 754 精度陷阱
- 掌握复数、布尔（`bool` 是 `int` 的子类）与各类型间的转换规则
- 会用 `divmod`、`round`、`abs`、`pow` 等数值内置函数与 `decimal` / `fractions` 精确计算
...
- [Python · 1变量](../1变量)
- 定长整数对照：[C 语言 · 数据](../../C/2数据)（溢出 vs 任意精度）

## 核心概念

Python 的整数不会溢出——它用变长数组存储任意位数，代价是运算比机器整数慢。浮点数则完全遵循 IEEE 754 二进制表示，`0.1 + 0.2 != 0.3` 不是 Python 的 bug 而是二进制无法精确表示十进制小数的必然结果。要精确算钱就得用 `decimal`，要精确算分数就用 `fractions`。

本章主线：
- 整数：任意精度、进制表示、位运算
- 浮点数：IEEE 754、精度误差与比较方式
- 复数与布尔类型
- 数值相关内置函数
...
整数任意精度、浮点必有误差、布尔是整数的子类——三条事实决定了 Python 数值运算的行为边界。涉及金额和精确比较时，改用 `decimal` 或 `math.isclose`。

## 练习

动手检验一下自己：

1. `0.1 + 0.2 == 0.3` 为什么是 `False`？给出两种正确判断浮点相等的写法。
2. `True + True` 的结果是什么？解释 `bool` 与 `int` 的继承关系带来的影响。
3. 计算 `2 ** 1000`，Python 为什么不会溢出？这与 C 的 `int` 有何本质区别？

## 易错点

- 浮点数不精确：`0.1 + 0.2 != 0.3`，比较浮点要用 `math.isclose` 或自定义容差。
- 整数除法方向与 C 不同：`-7 // 2` 在 Python 是 `-4`（向下取整），不是向零截断。
- `**` 优先级高于一元负号：`-2**2` 等于 `-4` 而非 `4`。
- 整数任意精度但很慢：Python 的 int 没有溢出上限，超大数运算会占用大量内存与时间。
- 虚数单位用 `j` 不是 `i`：写成 `1j`，`1i` 会报语法错误。

## 延伸阅读

- 上一篇：[Python · 1变量](../1变量)
- 下一篇：[Python · 2字符串](../2字符串)
- 返回 [Python 语言总览](../)
