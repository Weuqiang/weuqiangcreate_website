...
`<iframe>` 在当前页面中嵌入另一个完整文档，二者拥有独立的浏览上下文。sandbox 可限制嵌入页的能力（脚本、表单、同源等），降低第三方内容的风险。

本章主线：
- src 与独立浏览上下文
- sandbox 的权限收窄
- 跨域嵌入与 X-Frame-Options
...
`<iframe>` 把整页“画中画”嵌入，sandbox 是其安全围栏。嵌入不可信来源时务必启用 sandbox。

## 练习

动手检验一下自己：

1. 给嵌入第三方地图的 `<iframe>` 添加最小必要权限的 sandbox。
2. 解释父页脚本为何通常无法直接读取跨域 `<iframe>` 的内容。
