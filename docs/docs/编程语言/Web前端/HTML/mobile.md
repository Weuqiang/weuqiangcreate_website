...
移动端网页首先要声明 `<meta name="viewport">` 让页面按设备宽度渲染，否则手机会用桌面宽度缩放。核心思路是“响应式”：同一套 HTML，用 CSS 媒体查询在不同屏幕宽度下应用不同布局，配合弹性单位与图片自适应，实现一套代码适配手机、平板、桌面。
...
移动端靠 viewport + 响应式（媒体查询/弹性布局）一套代码适配多屏；关键点在于别让手机用桌面宽度缩放，并处理好触摸与图片自适应。
