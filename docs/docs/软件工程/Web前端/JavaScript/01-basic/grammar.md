...
- **`var` 提升与循环闭包**：`for (var i = 0; i < 3; i++) setTimeout(() => console.log(i), 0)` 会打印三次 `3`（终值），因为 `var` 是函数作用域且异步回调共享同一变量；改用 `let`（每次迭代独立绑定）或 IIFE 解决。
- **ASI 陷阱**：`return` 后直接换行写对象字面量会返回 `undefined`；行首以 `(`、`[`、`` ` `` 开头时，上一行末尾若省略分号会被合并成一条语句，务必显式加分号。
- **`==` 与 `===`**：`"1" == 1` 为 `true`，`null == undefined` 为 `true` 但 `null === undefined` 为 `false`；比较一律用 `===`。
- **`const` 不等于“不可变”**：`const` 只保证“绑定不可重新赋值”，对象/数组本身仍可修改（`const a = []; a.push(1)` 合法，`a = []` 才报错）；需要真正不可变请用 `Object.freeze` 或不可变库。
- **区块（`{}`）对 `var` 不创造作用域**：`{ var x = 1; }` 之后 `x` 仍可访问；只有 `let` / `const` 才有块级作用域。
- **`if`/`switch` 里的赋值笔误**：`if (x = 5)` 是赋值而非比较，常因少写 `=` 导致逻辑永远为真；养成 `if (5 === x)` 这种常量在左的写法可让笔误直接报语法错。
- **浮点精度**：`0.1 + 0.2 !== 0.3`（IEEE 754 二进制浮点误差），涉及金额用整数“分”或 decimal 库处理。
