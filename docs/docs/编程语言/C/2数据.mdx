...
- 理解整数/浮点/字符在内存中的二进制表示与取值范围
- 掌握进制转换、类型提升与隐式/显式类型转换规则
- 识别数据表示相关的典型陷阱（溢出、精度、符号位）
...
计算机只认二进制。所谓「数据类型」本质是「这段内存按什么规则解释」——同样的 0/1 比特，解释成有符号整数、浮点数还是字符，得到的值天差地别。理解表示是看懂一切运算结果的前提。

本章主线：
- 整型家族与取值范围（char/short/int/long/long long）
- 浮点数的 IEEE-754 表示与精度
- 进制与位级表示
- 类型转换：整型提升、隐式/显式转换
...
本章深入「数据在比特层面长什么样」：从各整型的字节数与取值范围，到浮点精度、进制与符号位，再到类型转换时的提升与截断规则。数据表示是 C 一切「诡异结果」的根源。

## 练习

动手检验一下自己：

1. `unsigned int x = 0; x--;` 的结果是什么？为什么？
2. 为什么不能直接用 `==` 比较两个 `double` 是否相等？
3. 写出十进制 `255` 的二进制、八进制、十六进制表示，并说明 char 如何复用整数运算。

## 易错点

- 整数除法会截断：`5 / 2` 得到 2 而不是 2.5，`1 / 3.0` 才是浮点除法；想要小数结果必须让至少一个操作数是浮点，如 `(double)a / b`。
- 用 `==` 比较浮点数：`0.1 + 0.2 == 0.3` 为假，因为 IEEE-754 无法精确表示 0.1；正确做法是判断两数之差的绝对值是否小于一个足够小的阈值。
- 有符号与无符号混合运算：`int i = -1;` 与 `unsigned u = 1;` 比较时，`i` 会被转换成一个巨大的无符号数，于是 `i > u` 成立；`strlen(a) - strlen(b) < 0` 永远为假也是同一个坑。
- `char` 是否带符号由实现决定：把 `char` 当小整数用时，值超过 127 的字节在有符号平台上会变成负数，需要明确写成 `signed char` 或 `unsigned char`。
- 整型溢出发生在提升之前：`int a = 100000; long long b = a * a;` 会先按 `int` 相乘再溢出（有符号溢出是未定义行为），必须写成 `(long long)a * a`。
- `printf` 的格式符与实参类型不符：用 `%d` 打印 `long long`、用 `%f` 打印 `int` 都是未定义行为，轻则输出乱码重则崩溃；`size_t` 要用 `%zu`，`long long` 要用 `%lld`。

## 易错点

- **整数除法截断**：`5 / 2` 等于 `2` 而非 `2.5`；要得到小数必须让操作数之一为浮点（`5.0 / 2`）。
- **浮点比较用 `==`**：因精度误差，应比较差值是否小于一个极小值 `fabs(a - b) < 1e-9`，而非直接相等。
- **字符与字符串混淆**：`'a'` 是单字符（整数），`"a"` 是含 `'\0'` 的字符串，类型与用法完全不同。
- **整型提升的规则**：`char` 参与运算会先提升为 `int`（有符号时做符号扩展），结果可能出乎意料。
- **无符号回绕**：无符号数下溢会变最大值（`0u - 1u == UINT_MAX`），易被当成逻辑 bug。
