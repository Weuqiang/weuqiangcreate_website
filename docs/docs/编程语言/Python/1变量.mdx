...
- 建立「变量是贴在对象上的标签」而非「装数据的盒子」这一心智模型
- 掌握命名规则、别名（多个名字指向同一对象）与 `id()` / `is` / `==` 的区别
- 理解小整数与短字符串的驻留机制，会用类型注解为动态类型补上契约
...
- [Python · 0基础语法](../0基础语法)
- 静态类型对照：[C 语言 · 变量](../../C/1变量)（定义即分配内存 vs 赋值即绑定引用）

## 核心概念

在 C 里 `int a = 1` 是「开一块 4 字节内存并写入 1」，在 Python 里 `a = 1` 是「先创建整数对象 1，再把名字 a 绑定到它」。这一差别决定了 Python 的赋值不复制数据、多个名字可以是同一对象的别名、`is` 比较身份而 `==` 比较值。驻留（interning）是解释器对小整数和短字符串做的复用优化，也是 `is` 偶尔「意外为真」的原因。
...
变量是引用而非容器：赋值绑定名字、别名共享对象、`is` 看身份 `==` 看值、驻留是优化而非语义保证。类型注解不影响运行期行为，但让动态类型的代码重新变得可读、可检查。

## 练习

动手检验一下自己：

1. 执行 `a = 256; b = 256; a is b` 与 `a = 257; b = 257; a is b`，结果为何不同？
2. `b = a` 之后修改 `b` 指向的列表，`a` 会变化吗？如果 `a` 是整数呢？解释背后的机制差异。
3. 类型注解 `def f(x: int) -> str:` 在运行时会强制检查吗？如果不会，它的价值体现在哪里？

## 易错点

- Python 变量是「名字绑定到对象」而非「盒子存值」：`a = b` 后两者指向同一对象，对可变对象（list/dict/set）原地修改会互相影响。
- 链式赋值 `a = b = []` 让两个名字指向同一个列表，`append` 会同时生效。
- 可变对象（list/dict/set）不能作函数默认参数，否则跨调用共享状态；用 `None` 哨兵替代。
- 函数内给全局变量赋值要先写 `global name`，否则只是新建了一个同名的局部变量。
- 命名遮挡内置：`list = [1, 2]` 会遮蔽内置 `list()`，后续调用直接报错。

## 延伸阅读

- 上一篇：[Python · 0基础语法](../0基础语法)
- 下一篇：[Python · 1数字](../1数字)
- 返回 [Python 语言总览](../)
- 跨语言对照：[Go](../../Go/) · [Rust](../../Rust/) · [C](../../C/)
- 跨主题关联·类型系统：[Rust 类型系统](../../Rust/类型系统/)（动态强类型 vs Rust 静态强类型：类型检查时机不同）
