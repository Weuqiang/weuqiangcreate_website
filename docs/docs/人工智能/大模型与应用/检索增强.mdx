...
- 切分粒度决定上限：切太碎丢上下文、切太大噪声多；理想是按语义边界（段落/标题）切，而非固定字数。
- 检索召回不准，生成再强也白搭：先盯住召回率（命中相关块的比例），再优化生成，否则垃圾进垃圾出。
- 未区分「检索内容」与「模型已知」：模型可能无视检索结果或把检索当真而编造，需显式要求「仅依据给定资料作答」。
- 知识库过期/冲突：文档更新后未重建索引，模型仍用旧版本回答，需建立索引刷新流程。
