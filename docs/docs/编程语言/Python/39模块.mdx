...
- 掌握 `import` 机制、包与 `__init__.py`、绝对导入与相对导入的区别
- 理解模块搜索路径（`sys.path`）与循环导入的成因
- 会用虚拟环境隔离依赖，能把自己的代码打包发布到 PyPI
...
- [Python · 10函数](../10函数)
- 工程工具：[开发工具链 · Git](../../../软件工程/开发工具链/Git)、[开发工具链 · Terminal](../../../软件工程/开发工具链/Terminal)
- 扩展开发：[C 语言 · Python与C互交互](../../C/19Python与C互交互)

## 核心概念

模块是「一个 .py 文件对应一个名字空间」，包是「带 `__init__.py` 的目录」。`import` 做三件事：按 `sys.path` 找到文件、执行一遍模块顶层代码、把模块对象绑定到当前名字空间。「执行一遍」是关键——它解释了为什么模块级代码只跑一次（有缓存 `sys.modules`），也解释了循环导入为什么会拿到半成品模块。
...
1. 访问 [PyPI 账户 Token 管理页面](https://pypi.org/manage/account/token/)
2. 选择 "Add API token"
3. 设置名称和权限范围
4. 复制生成的 token
...
模块解决命名冲突、包解决层级组织、虚拟环境解决依赖隔离、打包解决分发。理解「import 会执行模块顶层代码且只执行一次」，模块相关的怪现象就都能解释。

## 练习

动手检验一下自己：

1. `if __name__ == '__main__':` 的作用是什么？直接运行与被导入时 `__name__` 分别等于什么？
2. A 模块导入 B、B 模块又导入 A，会发生什么？给出两种打破循环导入的方法。
3. 解释绝对导入与相对导入的差异，说明为什么在脚本中使用相对导入常常失败。

## 易错点

- 循环导入：`a` 导入 `b`、`b` 又导入 `a` 会部分初始化失败，重构或用延迟导入解决。
- `from x import *` 污染命名空间：名字冲突难排查，显式 import 更清晰。
- 漏写 `if __name__ == '__main__':`：模块被 import 时也会执行主逻辑/测试。
- `sys.path` 顺序决定导入谁：同名模块可能遮蔽你想要的那个。
- 相对导入只在包内有效：顶层脚本用相对导入会报错，需用 `python -m` 运行。

## 延伸阅读

- 上一篇：[Python · 38代码执行与调试](../38代码执行与调试)
- 下一篇：[Python · 99练习](../99练习)
- 返回 [Python 语言总览](../)
