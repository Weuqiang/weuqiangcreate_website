...
sidebar_position: 5
title: Docker
description: "Docker 容器技术实战：安装与快速上手、镜像容器仓库三大概念、常用命令、数据卷与网络、docker-compose 多服务编排与 k8s 定位"
---
...
- 理解容器与虚拟机的本质差异，掌握镜像、容器、仓库三个核心概念
- 会用常用命令构建镜像、运行容器、管理数据卷与网络
- 能用 docker-compose 编排多服务应用，了解 k8s 的定位与边界

## 前置知识

在阅读下面的内容前，建议先掌握：

- 熟悉命令行操作：[Terminal](../Terminal)
- compose 文件格式：[YAML](../YAML)
- 理论支撑：[操作系统 · 进程、线程与调度](../../../计算机科学基础/操作系统/进程线程与调度)

## 核心概念

容器不是轻量虚拟机——它压根没有虚拟化硬件，本质是**被内核隔离和限制的普通进程**（namespace 管隔离、cgroup 管配额）。这解释了容器为什么秒级启动、为什么共享宿主机内核、为什么 Linux 容器不能直接跑在 Windows 内核上。镜像是分层的只读文件系统，容器是在镜像之上加一层可写层——所以容器删掉数据就没了，要持久化必须挂数据卷。
...
把一个 Python 服务打包成最小镜像，再构建运行，是最常见的 Docker 入手路径：

```dockerfile
# 最小 Python 镜像：构建并运行一个 Flask 服务
...
`Dockerfile` 描述「环境 + 启动命令」，`build` 把它固化为镜像，`run -p` 把容器端口映射到宿主机——同一镜像在任意装了 Docker 的机器上行为一致，这正是它解决「在我机器上能跑」问题的关键。

## 小结

容器 = 被隔离的进程，镜像 = 分层只读文件系统，容器的可写层是易失的。单机多服务用 compose，跨机集群才需要 k8s——不要为了用 k8s 而用 k8s。

## 易错点

- 容器里跑有状态数据却不挂卷：容器一删数据全没。
- 用 latest 拉镜像：不可复现，今天能跑明天挂。
- 构建上下文太大：`COPY .` 把 node_modules 也带进去，缓存频繁失效。
- 一条 RUN 装完不清理 apt 缓存：镜像虚胖，传输慢。
- 开发时把依赖装进容器却不挂源码卷：改一行要重建镜像。

## 练习

动手检验一下自己：

1. 容器与虚拟机在启动速度、资源占用、隔离强度上分别有何差异？根因是什么？
2. 容器重启后数据丢失，可能是什么原因？给出两种持久化方案。
3. 用 docker-compose 编排一个「Web 应用 + 数据库」的双服务栈，说明服务间如何互相访问。

## 延伸阅读

- 上一篇：[Terminal](../Terminal)
- 返回：[计算机科学基础总览](../../)
