---
title: "方式一"
description: "urllib 是一个收集了多个涉及 URL 的模块的自带包：可以打开和读取 URL、 抛出异常、解析"
---

## 学习目标

- 用 `urllib.request` 发起 GET/POST 并读取响应
- 处理 URL 编解码与异常（`URLError` / `HTTPError`）
- 知道 urllib 仅同步、HTTP/1.1，复杂场景选 `requests`
...
`urllib` 是标准库里最底层的 URL 处理包，分 `request`（打开 URL）、`parse`（解析/拼接）、`error`（异常）、`robotparser`。它同步阻塞、功能偏底层；日常爬虫/接口调用更推荐第三方 `requests`，但理解 urllib 有助于看懂其原理与 `Request`/`urlopen` 的语义。
...
with urllib.request.urlopen("https://httpbin.org/get") as resp:
    ...
    print(resp.status, body[:80])

# 带查询参数与自定义请求头
url = "https://httpbin.org/post?" + urllib.parse.urlencode({"q": "python"})
...
- 默认同步阻塞：一个大响应会卡住整个线程，高并发应换 `requests`+线程 或 `aiohttp`。
- 状态码非 2xx 不一定抛异常：`urlopen` 对 4xx/5xx 抛 `HTTPError`，但要用 `try` 捕获，别假设一定成功。
- 中文/特殊字符要 `urlencode`：直接拼 URL 会因编码错误 400。
- 响应体是字节：记得 `.decode("utf-8")`，否则是 bytes。
- 超时要用 `timeout=`：默认可能无限等待。

## 练习

- 用 urllib 抓取一个页面并统计链接数量。
- 处理 `URLError` 与 `HTTPError` 两种异常。
- 用 `urllib.parse` 解析并重组一个 URL。

## 小结

`urllib` 是标准库最底层的 HTTP 客户端，够用但偏繁琐。理解 `Request`/`urlopen`/`urlencode` 后，日常开发更推荐 `requests`（语法更友好）；但对「零依赖」场景，urllib 仍是可靠选择。

## 延伸阅读

- [网络与通信](..)
- [asyncio](../asyncio)
- 跨主题关联：[网络分层与 TCP/IP](../../../../../计算机科学基础/计算机网络/网络分层与TCP与IP)（urllib 的请求最终跑在 TCP/IP 上：懂分层才知道超时与重试该在哪一层处理）
