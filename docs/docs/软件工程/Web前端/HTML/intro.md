...
- **标签必须正确闭合**：漏掉结束标签（如忘写 `</p>`），浏览器虽会容错，但后续内容可能被“吞进”该元素，布局与脚本结果不可预期。
- **属性值一定要加引号**：HTML5 允许不加引号，但只要值里出现空格、`>`、`=` 等字符就会解析崩坏；养成双引号习惯最稳。
- **空元素不要画蛇添足**：`<br>`、`<hr>`、`<img>`、`<input>` 等无内容标签不要写结束标签（如 `</br>`），也不要在它们内部放文本。
- **`lang` 与 `charset` 别漏**：`<meta charset="utf-8">` 决定中文是否乱码；`lang="zh-CN"` 影响屏幕阅读器与翻译，二者都应写在 `<head>` 顶部。
- **`<p>` 里别塞 block 元素**：`<p>` 只允许“短语内容”，把 `<div>`、`<ul>` 等块级元素放进去，浏览器会提前闭合 `<p>`，结构会悄悄错位。
- **注释不能嵌套**：`<!-- ... <!-- 内层 --> ... -->` 中第一个 `-->` 就会结束注释，内层 `--` 也会提前截断，写多行注释时避免行内出现 `--`。
