...
- 分清并发与并行、IO 密集与 CPU 密集，判断何时该用异步
- 掌握 `async def` / `await` 语法与事件循环的单线程协作式调度模型
- 会用 `asyncio` 的任务、超时、并发聚合处理真实的网络 IO 场景

## 前置知识

在阅读下面的内容前，建议先掌握：

- [Python · 10函数](../10函数)、[Python · 25迭代器](../25迭代器)（协程由生成器演化而来）
- 理论支撑：[进程、线程与调度](../../../计算机科学基础/操作系统/进程线程与调度)
- 横向对照：[Go · goroutine与channel](../../Go/并发模型/goroutine与channel)（有栈协程 + 运行时调度）

## 核心概念

异步不是多线程。`asyncio` 是**单线程**的协作式并发：事件循环在一个线程里轮流推进多个协程，每个协程在 `await` 处主动让出控制权。这意味着它只能加速 IO 等待（网络、磁盘），对 CPU 密集任务毫无帮助——反而会因为某个协程不让出而阻塞整个循环。选型口诀：IO 密集用 asyncio，CPU 密集用多进程。

本章主线：
- 同步阻塞的代价与并发模型选型
- `async def` / `await` 与协程对象
- 事件循环、任务与并发聚合
- `asyncio` 常用 API 与超时控制
...
异步编程用「主动让出」换取「高并发 IO」，代价是整条调用链都必须是异步的（函数染色）。判断标准只有一条：瓶颈是在等 IO 还是在算数据。

## 练习

动手检验一下自己：

1. 在协程里调用 `time.sleep(1)` 会发生什么？应该改用什么？
2. `await` 一个协程与用 `asyncio.create_task` 包装后再 `await`，并发行为有何不同？
3. 为什么说 asyncio 对 CPU 密集任务无效？结合 GIL 与事件循环解释。

## 示例

下面演示 `async` / `await` 与 `asyncio`：并发下载三个「任务」，总耗时约等于最慢的那个而非三者之和。
...
async def task(name, secs):
    print(f"{name} 开始")
    await asyncio.sleep(secs)       # 非阻塞等待，让出事件循环
    print(f"{name} 结束（{secs}s）")
    return name
...
- `async def` 里忘写 `await`：协程不会被真正执行，只是返回一个协程对象，毫无效果。
- 阻塞调用会卡死整个事件循环：`time.sleep` 不是 `await asyncio.sleep`，必须用后者。
- 混用同步库：`requests.get` 在 async 函数里仍然是阻塞的，I/O 密集型应改用 `aiohttp`。
- `asyncio.run` 不能嵌套：已在事件循环内再调用它会报错，应直接 `await`。
- 只 `create_task` 却没 `await`：任务可能在事件循环关闭前被取消，记得等待其完成。

## 延伸阅读

- 上一篇：[Python · 29文件操作](../29文件操作)
- 下一篇：[Python · 38代码执行与调试](../38代码执行与调试)
- 返回 [Python 语言总览](../)
- 跨语言对照：[Go](../../Go/) · [Rust](../../Rust/) · [C](../../C/)
- 理论对照：[进程、线程与调度](../../../计算机科学基础/操作系统/进程线程与调度)（事件循环是单线程并发，对照 OS 多线程调度）
