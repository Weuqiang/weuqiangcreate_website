---
title: "事件模型"
description: "浏览器的事件模型，就是通过监听函数（listener）对事件做出反应。事件发生后，浏览器监听到了这个"
---
...
- [事件基础](/docs/软件工程/Web前端/JavaScript/events/common)

## 核心概念

现代 DOM 事件流为捕获、目标、冒泡三阶段。早期有 0 级（on* 属性）与 2 级（addEventListener）模型，3 级规范完善了细节。
...
事件流是捕获、目标、冒泡；用 addEventListener 的第三参数控制捕获阶段。

## 练习

动手检验一下自己：

1. 用第三参数 true/false 在捕获与冒泡阶段分别监听。
2. 说明 0 级与 2 级模型的差异。
