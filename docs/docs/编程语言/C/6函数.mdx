...
函数是「可复用的代码块」，通过参数接收输入、用返回值产出输出。C 默认「值传递」——形参是实参的拷贝；想修改外部数据必须传地址。递归则是函数调用自身，依赖调用栈保存每一层现场。
...
本章把大程序拆成小函数：定义/声明分离、值传递与地址传递的取舍、作用域边界，以及递归这一「自己调用自己」的强大但易溢出的技术。函数即模块化的基本单元。

## 练习

动手检验一下自己：

1. 为什么数组名作为函数参数时，函数内无法用 `sizeof` 算出元素个数？
2. 写一个递归函数求阶乘，并说明当 n 过大时会因为什么而崩溃。
3. 值传递下，函数内修改形参为什么不影响实参？用内存图解释。

## 易错点

- 以为值传递能改实参：`void swap(int a, int b)` 交换的只是形参副本，函数返回后实参毫无变化；必须写成 `void swap(int *a, int *b)` 并在函数内解引用。
- 数组参数会退化为指针：`void f(int arr[])` 里的 `arr` 实际是 `int *`，`sizeof(arr)` 得到的是指针大小，数组长度必须用额外的参数传进来。
- 调用前既没有函数原型也没有定义：老标准下编译器按隐式规则猜返回类型和参数，实参类型不匹配时结果错得毫无头绪；所有函数都应先声明后使用。
- 返回局部变量的地址：函数返回后栈帧失效，`return &local;` 拿到的是悬空指针，返回局部数组同理；需要返回缓冲区就让调用方传进来，或用 `malloc` 分配并约定由谁释放。
- 递归缺少基线条件或基线条件永远到不了：每层调用都占一个栈帧，无限递归会迅速把栈耗尽并崩溃，而不会像死循环那样一直挂着。
- `int f()` 与 `int f(void)` 含义不同：前者表示「参数列表未指定」，传任意个实参都不会报错；要表达「不接受参数」必须显式写 `void`。

## 易错点

- **参数类型不匹配**：C 对参数检查宽松，传错类型（尤其窄类型与指针）可能静默产生垃圾结果。
- **数组退化为指针**：数组作为函数参数时退化为指针，函数内 `sizeof(arr)` 得到指针大小而非数组长度，长度要单独传。
- **返回局部变量地址**：返回函数内局部数组/变量的指针，调用结束后那块内存已无效，解引用是未定义行为。
- **传值与传址混淆**：想在函数内修改调用者的变量必须传指针，否则只是改了副本。
- **递归缺基准情形**：没有正确的递归终止条件会让栈无限增长直至栈溢出。
- **变参函数类型错配**：`printf` 的格式符与实参类型/数量不符，会读出错误甚至崩溃的数据。
...
- 跨主题关联：[递归与分治](../../../计算机科学基础/数据结构与算法/递归与分治)（递归的构成与分治思想：C 递归函数的栈帧、基线条件在此看数学定义）
- 跨主题关联·类型系统：[Rust 类型系统](../../Rust/类型系统/)（C 隐式类型转换的坑，Rust 用强类型与显式 `as` 转换规避）
