...
- 用 `<video>` / `<audio>` 嵌入媒体并控制播放
- 理解 `<source>` 多格式回退与 `<track>` 字幕
- 知道 `<embed>/<object>` 的用途与局限
...
HTML5 让音视频成为原生能力，不再依赖 Flash 等插件：`<video>` 嵌入视频、`<audio>` 嵌入音频，可用 `controls` 显示控件、`autoplay` 自动播放；`<source>` 提供多种格式让浏览器自选，`<track>` 提供字幕。`<embed>/<object>` 用于嵌入外部资源，现代场景已较少使用。

本章主线：
- `<video>`：视频嵌入与控制
- `<audio>`：音频嵌入
- `<track>` 字幕与 `<source>` 多格式
- `<embed>` / `<object>` 的用途
...
HTML5 用 `<video>/<audio>` 原生播放媒体，`<source>` 提供格式回退、`<track>` 提供字幕；无需插件即可在页面中集成音视频。

## 练习

动手检验一下自己：

1. 写一个带控件、支持 mp4 与 webm 两种 source 的视频播放器。
2. 为视频添加中文字幕轨道 `<track>`。
3. 对比 `<video>` 与原生 `<object>` 嵌入 Flash 的优劣。
