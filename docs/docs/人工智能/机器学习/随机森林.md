...
sidebar_position: 4
title: 随机森林
description: "理解bagging与双重随机如何把多棵决策树变成鲁棒易用的默认基线模型。"
---
...
- 理解 bagging 与「行采样+列采样」双重随机如何降低方差
- 掌握多棵树投票/平均的输出方式与概率估计
- 认识袋外（OOB）误差与特征重要度的免费红利

## 前置知识

在阅读下面的内容前，建议先掌握：

- 先读 [集成学习Boosting](../集成学习Boosting) 对照两种集成思路
- 树的基础见 [分类算法](../分类算法) 的决策树部分

## 核心概念

随机森林是「很多棵决策树，各自看不同的样本和特征，最后投票」。两重随机（bootstrap 行采样 + 列采样）让树之间尽可能不相关，平均下来方差比单棵树小很多、不易过拟合。它还能免费给出袋外（OOB）误差和特征重要度，是工业界最爱用的「开箱即用」模型之一。
...
前文对比了梯度提升，这里补上主角「随机森林」本身的最小用法，并顺便看特征重要性。

```python
# 随机森林分类：基学习器为决策树，多棵树投票降低方差
from sklearn.datasets import load_iris
from sklearn.ensemble import RandomForestClassifier
from sklearn.model_selection import train_test_split
from sklearn.metrics import accuracy_score
...
rf.fit(X_tr, y_tr)
print("准确率:", accuracy_score(y_te, rf.predict(X_te)))
...
随机森林在每棵树训练时随机选特征、随机抽样本（Bootstrap），靠「多样性」抑制单棵决策树的过拟合，因此对异常值不敏感、几乎不用调参即可拿到稳健基线。

## 小结

随机森林用「多树投票+双重随机」把方差压下去，鲁棒、易用、能给特征重要度。它不像 boosting 那样需精细调参也能不错，是很好的默认基线。

## 易错点

- 森林本身不易过拟合：靠「多棵 + 随机」抵消单棵过深，真正要调的是树的数量与每棵随机性（max_features）。
- 特征重要性会被相关特征稀释：两个高度相关特征会平分重要性，误以为都不重要。
- 类别不平衡下仍会偏向多数类，需设 class_weight。
- 森林可解释性弱于单棵树：想解释单条预测要用置换重要性或 SHAP，而非直接读树。

## 练习

动手检验一下自己：

1. 随机森林为什么比单棵决策树更不易过拟合？
2. OOB 误差是什么，它省掉了什么（相比单独留验证集）？
3. 特征重要度高一定能说明「因果重要」吗？举一个反例。
