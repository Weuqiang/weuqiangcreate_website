---
title: "函数"
description: "函数是一段可以反复调用的代码块。函数还能接受输入的参数，不同的参数有唯一对应的返回值。"
---
...
- [JavaScript 语法基础](/docs/软件工程/Web前端/JavaScript/basic/grammar)

## 核心概念

函数是可调用的对象，可作为值传递（一等公民）。箭头函数不绑定自己的 this，适合回调；普通函数的 this 由调用方式决定。
...
函数是一等公民；箭头函数无自身 this，普通函数 this 由调用方决定，闭包让函数记住定义时的作用域。

## 练习

动手检验一下自己：

1. 用箭头函数写求和回调，对比普通函数里 this 的指向。
2. 写一个返回内部计数器的闭包函数。

## 延伸阅读

- 跨主题关联：[递归与分治](../../../../../计算机科学基础/数据结构与算法/递归与分治)（递归的构成与分治思想：JS 递归斐波那契在此看数学定义与栈溢出）
- 跨主题关联·类型系统：[Rust 类型系统](../../../../../编程语言/Rust/类型系统/)（JS 动态弱类型的隐式转换，对照 Rust 静态强类型的安全）
...
- Juriy Zaytsev, [Named function expressions demystified](https://kangax.github.io/nfe/)
- Marco Rogers polotek, [What is the arguments object?](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/arguments)
- Juriy Zaytsev, [Global eval. What are the options?](https://perfectionkills.com/global-eval-what-are-the-options/)
- Axel Rauschmayer, [Evaluating JavaScript code via eval() and new Function()](https://www.2ality.com/2014/01/eval.html)
