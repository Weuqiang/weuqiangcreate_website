...
setTimeout/setInterval 把回调推迟到未来执行，属于宏任务。实际延迟受事件循环与最小延迟（通常 4ms）影响，嵌套过深会被限流。
...
定时器回调是宏任务，延迟并非精确；记得在组件卸载时清除。

## 练习

动手检验一下自己：

1. 用 setTimeout 实现轮询并能在条件满足时清除。
2. 说明 setInterval 与事件循环的关系。
