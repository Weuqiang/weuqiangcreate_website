...
- N+1 查询：循环里查库，一次接口打几百次 SQL，应用层卡死。
- 索引失效：对列套函数、隐式类型转换、前导模糊查询，都会让索引用不上，退化成全表扫描。
- 事务过大/过长：锁表 + 连接池耗尽，把数据库拖垮。
- 用字符串存 JSON/时间：丢失类型约束与索引能力，查询只能靠 LIKE。
- 不设连接池上限：突发流量把 DB 连接瞬间打满，全站不可用。
