...
sidebar_position: 28
title: 装饰器与作用域
description: "Python 装饰器与作用域详解：@ 语法、functools.wraps、单例与多重装饰器、装饰器工厂，以及 LEGB 作用域、global、nonlocal 与闭包等要点"
---
...
- 掌握 LEGB 作用域查找规则与 `global` / `nonlocal` 的使用时机
- 理解闭包如何捕获外层变量，能解释「循环里创建闭包」的经典陷阱
- 会写无参装饰器、带参装饰器与类装饰器，并用 `functools.wraps` 保留元信息

## 前置知识

在阅读下面的内容前，建议先掌握：

- [Python · 10函数](../10函数)（函数是一等对象、闭包）

## 核心概念

装饰器不是新语法，只是语法糖：`@deco` 等价于 `f = deco(f)`。它之所以成立，全靠两个前提——函数是一等对象（可以被传递和返回）和闭包（内层函数记住外层变量）。作用域的 LEGB 规则（Local → Enclosing → Global → Builtin）决定了名字从哪找起，`global` 和 `nonlocal` 则是显式打破这一默认查找的开关。

本章主线：
- 装饰器原理与 `@` 语法糖
- 带参装饰器与类装饰器
- `functools.wraps` 与元信息保留
- LEGB 作用域、`global` 与 `nonlocal`
...
作用域回答「名字从哪找」，闭包回答「变量怎么被记住」，装饰器则把二者组合成了「不改源码就增强函数」的能力。写装饰器时别忘了 `@functools.wraps`。

## 练习

动手检验一下自己：

1. 在函数内给全局变量赋值为什么会报 `UnboundLocalError`？如何修复？
2. 不加 `functools.wraps` 的装饰器会丢失被装饰函数的哪些信息？
3. 写一个带参数的装饰器 `@retry(times=3)`，失败时自动重试指定次数。

## 易错点

- 装饰器在**定义时**执行，不是调用时：模块加载时就会运行装饰器逻辑。
- 闭包变量陷阱同函数：装饰器内层引用外层循环变量要即时绑定（默认参数法）。
- 别忘了 `functools.wraps`：不加会丢失原函数的 `__name__` / `__doc__`，调试和日志都看不清。
- 多层装饰器执行顺序：从离函数最近的开始包裹，调用时由外到内；写错顺序结果会反。
- 带参装饰器多一层嵌套：`@deco(arg)` 返回的是真正的装饰器，容易少写一层函数。

## 延伸阅读

- 上一篇：[Python · 25迭代器](../25迭代器)
- 下一篇：[Python · 29文件操作](../29文件操作)
- 返回 [Python 语言总览](../)
