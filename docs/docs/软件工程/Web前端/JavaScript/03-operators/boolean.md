---
title: "布尔运算符"
description: "取反、且、或、三元四个布尔运算符：六个取反为真的假值、双重取反转布尔、&& 与 || 的短路求值和默认值写法，以及三元表达式与 if...else 的差别。"
---
...
&& 与 || 不总是返回布尔，而是返回第一个决定结果的操作数（短路）。常用于默认值（a || b）与守卫（a && a.b）。
