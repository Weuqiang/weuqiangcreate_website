...
- 跨主题关联·抽象：[设计模式](../../../软件工程与后端/工程实践/设计模式)（Python 不需要 interface 关键字，用鸭子类型与 ABC 表达同样的抽象）
- 跨语言抽象对照：[Java 接口与多态](../../../编程语言/Java/语言基础/面向对象与核心语法)（静态类型的显式契约）· [JavaScript 原型继承](../../../编程语言/Web前端/JavaScript/oop/prototype)（同为动态语言，但用原型链而非类继承树）
