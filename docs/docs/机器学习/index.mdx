...
AI 就是让机器能感知、学习、推理。机器学习（ML）是实现 AI 的核心方法——喂数据，让机器自己找规律。

传统 ML 的套路：数据 → 特征 → 模型 → 预测。现在深度学习火了，神经网络成了主流。

先看个简单例子，用加减乘除求平方根（这就是机器学习的缩影）：
...
现在做深度学习，PyTorch 基本是首选。生态太强了。

YOLOv11、Hugging Face Transformers、Stable Diffusion 都在用 PyTorch。很多开源大模型也是基于 PyTorch 搭的。

PyTorch 能调 GPU/NPU 加速，需要装对应的 CUDA 版本。简单说，PyTorch 通过 CUDA 驱动显卡干活。
...
WSL2 跑 CUDA 会有性能损失。同一块 3090 Ti，Windows 和 WSL2 读写速度差挺多：
...
# WSL2 Ubuntu-24.04
name: NVIDIA GeForce RTX 3090 Ti
write speed: 2632.96 MB/s  # 写入慢了近一半
read speed: 4429.29 MB/s   # 读取也有损失
```

追求性能就直接在 Windows 或 Linux 物理机上跑。
### torch框架
