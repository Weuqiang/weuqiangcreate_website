...
sidebar_position: 0
title: LSTM
description: "从RNN的长依赖困境到LSTM/GRU门控，理解序列记忆是怎么被显式管理起来的。"
---
...
- 理解 RNN 用隐藏状态传递记忆，以及长依赖下的梯度问题
- 掌握 LSTM 输入门/遗忘门/输出门对记忆的显式控制
- 认识 GRU 作为更省参数简化版的取舍
...
- 后续主流见 [Transformer](../Transformer)（同目录）
- 梯度基础见 [梯度与方向导数](/docs/数学基础/微积分/梯度与方向导数)

## 核心概念

普通 RNN 把「上一步的隐藏状态」当作记忆传给下一步，天然适合序列。但它记不住久远信息——梯度沿时间反传时要么消失要么爆炸，长依赖学不到。LSTM 用「输入门/遗忘门/输出门」显式控制记住什么、忘掉什么，像给网络配了个可写可读的内存单元；GRU 是更省参数的简化版。门控让长程信息能稳定流过很多时间步。
...
下面用一个最小 LSTM 做「正弦序列预测」：用过去 10 个时刻预测下一时刻，演示 `nn.LSTM` 的输入输出形状。

```python
# 极简 LSTM：用历史正弦序列预测下一时刻（序列回归）
import numpy as np
import torch
import torch.nn as nn
...
X = np.stack([y[i:i+10] for i in range(len(y)-10)])   # 窗口长 10
Y = y[10:]
...
`batch_first=True` 时输入形状为 `(batch, seq_len, input_size)`；`out[:, -1, :]` 取序列最后一步的隐藏状态作为整段序列的编码，再交给全连接层输出预测值。

## 小结

LSTM 用门控把「记忆」显式管理起来，解决了普通 RNN 忘性大、长依赖学不到的痛点。它曾主导序列建模，直到 Transformer 用注意力把它在很多任务上取代。

## 易错点

- LSTM 缓解了但没根除梯度消失：超长程依赖仍可能衰减，超长序列考虑 Transformer。
- 变长序列要统一长度或做 masking：直接 pad 不处理会让模型学到 padding 规律。
- 别把 LSTM 当万能时序模型：对强季节性/趋势，朴素统计或时序专用模型可能更简单有效。
- 双向 LSTM 不能用在「未来不可见」的场景（如实时预测），否则泄露未来信息。

## 练习

动手检验一下自己：

1. 为什么普通 RNN 学不到长依赖，LSTM 能？遗忘门的作用是什么？
2. 梯度爆炸在 RNN 上怎么表现、怎么治（如梯度裁剪）？
3. GRU 比 LSTM 少了哪个门、代价是什么？

## 延伸阅读

- 下一篇：[Transformer](../Transformer)
- 返回：[本栏目总览](../)
