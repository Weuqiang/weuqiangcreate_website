...
- 理解 `<a>` 作为超链接在 Web 互联中的核心作用
- 掌握 href、target、rel 等核心属性的含义与正确用法
- 能用页内锚点实现站内定位，并用 rel 规避新窗口安全风险

## 前置知识

在阅读下面的内容前，建议先掌握：

- [HTML 概览](/docs/软件工程/Web前端/HTML/intro)
- [URL 与编码](/docs/软件工程/Web前端/HTML/url)

## 核心概念

`<a>`（anchor）是超链接元素，通过 href 指向目标资源，是超文本网络互联的基础。它可以链接到其它页面、站内锚点，或 mailto:/tel: 等协议。target 控制打开方式，rel 描述当前文档与目标的关系（如 noopener/noindex）。

本章主线：
- href 的多种取值：绝对/相对 URL、锚点、协议
- target 与 rel 配合控制新窗口行为
- 可访问性：链接文案应自解释
...
`<a>` 用 href 回答“去哪里”，用 target/rel 回答“怎么去”。打开外部站点到新窗口时务必加 rel="noopener"，避免反向控制原页面。

## 练习

动手检验一下自己：

1. 写一个指向页内 #top 锚点的返回顶部链接。
2. 用 target="_blank" 打开外部站点，并说明为何要加 rel="noopener"。
