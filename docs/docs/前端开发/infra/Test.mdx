...
测试覆盖率是衡量代码测试完整性的重要指标，它表示测试用例执行时覆盖了多少源代码。

### Istanbul

Istanbul 是 JavaScript 代码覆盖率工具的标准，它可以生成详细的覆盖率报告。

#### 覆盖率类型

1. **语句覆盖率 (Statement Coverage)**: 测试是否执行了每一行代码
2. **分支覆盖率 (Branch Coverage)**: 测试是否执行了每个条件分支
3. **函数覆盖率 (Function Coverage)**: 测试是否调用了每个函数
4. **行覆盖率 (Line Coverage)**: 测试是否执行了每一行

#### 使用方式

```bash
# 安装 nyc (Istanbul 的命令行工具)
