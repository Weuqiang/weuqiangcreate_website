...
想知道一个东西是什么类别？看看它周围的邻居都是啥，少数服从多数。

具体步骤：
1. 记住所有训练数据（对，就是死记硬背）
2. 来了新数据，算它和所有老数据的距离
3. 找出距离最近的K个邻居
4. 看这K个邻居里哪个类别最多，就预测是那个类别

虽然也能做回归，但实际工作中主要用来做分类。
...
SVM的思路是：**找一条最优的分界线，把不同类别的数据分开**。

想象一下：
- 把每个数据当成空间中的一个点
- 有几个特征，就是几维空间（2个特征=2D平面，3个特征=3D空间）
- 比如用身高和头发长度来分男女，就是在2D平面上画一条线

SVM要找的不是随便一条线，而是**离两边数据都最远的那条线**（最大间隔）。这样分类效果最稳定。
