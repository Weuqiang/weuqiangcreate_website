...
sidebar_position: 15
title: 高级数据表示
description: "静态链表与动态链表的头插法、尾插法实现，链表的查找、删除、销毁与长度计算，以及字符串的初始化、输出与常用处理函数"
---
...
- 掌握用结构体+指针构建链表、树等动态数据结构
- 理解可变长数据与不透明类型（抽象数据类型）
- 能把现实关系建模成内存中的节点网络
...
- [结构与其他数据形式](../12结构和其他数据形式)
- [指针与动态内存](../10存储类别、链接和内存管理)

## 核心概念

当结构体里嵌入「指向下一个同型结构的指针」，静态数组就升级成可在运行时自由生长的动态结构——链表、树、图皆源于此。节点靠 `malloc` 散落堆上，靠指针串成网，生死全由你掌控。
...
- 链表实现了，内存零碎数据的有效组织。比如，当我们用 malloc 来进行内存申请的时候，当内存足够，但是由于碎片太多，没有连续内存时，只能以申请失败而告终，而用链表这种数据结构来组织数据，就可以解决上类问题。
  ![链表示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/eba20792779d9d9a99e7d4ff0d23ef13.png)

### 静态链表

![静态链表示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/44a5722c917071e937da435fe5695a26.png)
...
* 头指针带了一个空链表节点, 空链表节点中的 next 指向 NULL
  ![空链表示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/21a038cfb75cce739e0f1d333126db74.png)
...
* 头指针带了一个非空节点, 最后一个节点中的 next 指向 NULL
  ![非空链表示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/4f238221615ee2ef930526bd53cdc0ef.png)
...
本章把指针与结构体合体成真正的「动态数据结构」：链表让数据在堆上自由生长，树用递归定义层级关系。这是从「固定数组」迈向「通用容器库」的关键一跃。

## 练习

动手检验一下自己：

1. 为什么链表节点通常用 `malloc` 分配而不是声明为局部变量？
2. 用结构体自引用定义一个单向链表节点，并写出头插法。
3. 链表相比数组，在插入/删除与随机访问上各有什么优劣？

## 示例

下面是一个带头节点的单向链表的完整生命周期：创建、尾插、按值删除、遍历、整体销毁。重点在删除与销毁时「先保存后继指针，再 `free` 当前节点」这个固定套路。
...
int push_back(Node *head, int data) {
    ...
    cur->data = data;
    cur->next = NULL;          /* malloc 不清零，必须显式赋值 */
...
/* 删除第一个值等于 key 的节点：先摘链，再释放 */
int remove_first(Node *head, int key) {
    Node *pre = head;
    while (pre->next != NULL && pre->next->data != key) pre = pre->next;
    if (pre->next == NULL) return -1;      /* 没找到 */
...
int main(void) {
    ...
    for (int i = 1; i <= 5; i++) push_back(head, i * 10);
    print_list(head);           /* 10 -> 20 -> 30 -> 40 -> 50 -> NULL */

    remove_first(head, 30);
...
- `free(p)` 之后再访问 `p->next` 是典型的 use-after-free：释放前必须先用临时变量保存后继指针，销毁整条链表时最容易踩这个坑。
- 忘记检查 `malloc` 的返回值：内存不足时它返回 `NULL`，紧接着执行 `cur->data = x` 就是对空指针写入，直接段错误。
- 新节点的 `next` 没赋值：`malloc` 得到的内存是未初始化的随机字节，尾插时若漏写 `cur->next = NULL;`，遍历会跑向野地址。
- 头插法的两行顺序写反：必须先 `cur->next = head->next;` 再 `head->next = cur;`；反过来会把后面整条链丢掉，既断链又泄漏内存。
- 混淆「带头节点」与「不带头节点」两种写法：带头节点时遍历要从 `head->next` 开始，`head` 本身不存数据；两者混用会多算或漏算一个元素。
- 删除节点时只改指针不 `free`（内存泄漏），或只 `free` 不修正前驱的 `next`（留下悬空指针）——这两个错误经常成对出现。

## 延伸阅读

- 上一篇：[`14C预处理器和C库`](../14C预处理器和C库)
- 下一篇：[`17并发执行`](../17并发执行)
- 返回 [C 语言总览](../)
