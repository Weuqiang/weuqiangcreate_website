...
sidebar_position: 1
title: Transformer
description: "Transformer 的推理循环与词嵌入落地：分词、构建词表与嵌入矩阵初始化，独热编码和 nn.Embedding 查表两种取向量方式对比，并叠加位置编码。"
---
...
- 理解自注意力让任意两位置一步互通，不再受时序衰减
- 掌握 Q/K/V、相似度权重与多头并行的含义
- 认识位置编码的必要性及它带来的并行训练优势

## 前置知识

在阅读下面的内容前，建议先掌握：

- 先读 [序列处理/LSTM](../LSTM)（同目录）看被取代的对象
- 矩阵基础见 [矩阵与线性方程组](/docs/数学基础/线性代数/矩阵与线性方程组)

## 核心概念

Transformer 抛弃循环，用自注意力让序列里每个位置直接「看」所有位置——一句话里第 1 个词和第 100 个词的距离始终是 1 步，不再受时序衰减。它把输入做成 Query/Key/Value 三份，算 Query 与所有 Key 的相似度得到权重，再加权平均 Value；多头是并行多组这样的注意力捕捉不同关系。因为没有循环，它能完全并行训练，也催生了大模型。
...
把上文的词嵌入与位置编码之后，Transformer 的核心运算是「缩放点积注意力」。下面用 NumPy 实现一个无依赖版本，直观看到注意力权重如何分配。
...
def scaled_dot_product_attention(Q, K, V):
    d_k = Q.shape[-1]
    scores = (Q @ K.T) / np.sqrt(d_k)     # 缩放，避免点积过大导致梯度消失
    exp = np.exp(scores - scores.max(axis=-1, keepdims=True))
    attn = exp / exp.sum(axis=-1, keepdims=True)   # softmax
    return attn @ V, attn

Q = np.array([[1.0, 0.2], [0.3, 1.0]])
K = np.array([[1.0, 0.1], [0.2, 0.9]])
V = np.array([[0.5, 0.5], [0.8, 0.2]])
out, weights = scaled_dot_product_attention(Q, K, V)
print("输出:\n", out)
print("注意力权重:\n", weights)
```

缩放因子 `1/sqrt(d_k)` 让点积的方差与维度无关，softmax 不会被推到饱和区；多头注意力只是把 Q/K/V 投影成多组、并行算完再拼接，从而在不同子空间捕捉不同类型的关系。

## 小结

Transformer 用自注意力把「任意两位置的关系」一步算清，且可完全并行，因此既能训得大又能捕捉长依赖。它是现代大模型的共同骨架。

## 易错点

- 自注意力是 O(n²) 复杂度：序列长度翻倍，计算与显存约翻四倍，长文本要分段或用稀疏/线性注意力。
- 位置编码不能丢：自注意力本身对顺序不敏感，没有位置编码模型不知道词序，翻译/语法会乱。
- 预训练与下游任务分布 mismatch：直接拿基础模型做特定任务不微调，效果往往不如针对性微调。
- 推理时 KV cache 是显存大户：长对话/长文档生成要关注 context 长度与缓存管理。

## 练习

动手检验一下自己：

1. 自注意力相对 RNN 最大的两个优势是什么（并行、长依赖）？
2. 没有位置编码，Transformer 会缺什么能力？为什么？
3. 多头注意力里「多头」到底在并行学什么不同的东西？

## 延伸阅读

- 上一篇：[LSTM](../LSTM) · 下一篇：[Vision Transformer](../Vision Transformer)
- 返回：[本栏目总览](../)
- 相关：[机器学习](../../../机器学习/) · [深度学习·计算机视觉](../../../深度学习/计算机视觉/)
- 返回：[人工智能总览](../../../)
