...
sidebar_position: 1
title: 聚类算法
description: "从无标签数据的划分出发，理解K-Means、层次聚类与DBSCAN的假设与适用。"
---
...
- 理解聚类是无标签学习，目标是把相似样本聚成一簇、把不相似的分开
- 掌握 K-Means 的迭代（指派-更新）、K 的选择与初始化敏感性
- 认识层次聚类与 DBSCAN 对簇形状/密度的不同假设与适用边界

## 前置知识

在阅读下面的内容前，建议先掌握：

- 先读 [分类算法](../分类算法) 对照有标签与无标签两种范式
- 距离度量依赖 [向量空间与基](/docs/数学基础/线性代数/向量空间与基)

## 核心概念

聚类在「没有标准答案」的数据里自己找结构。K-Means 假定簇是球状、靠质心迭代收敛；层次聚类不预设 K、用树状合并看不同粒度；DBSCAN 换思路，按密度连通性把稠密区连成簇、把孤立点当噪声，因此能找任意形状且不要求指定簇数。选哪种，取决于你的簇是球形、嵌套还是密度不均。
...
- 先标准化再聚类：聚类靠距离，量纲不同会让数值大的特征独大，结果被单一维度主导。
- K 值怎么定是难题：肘部法/轮廓系数是经验而非真理，业务可解释性往往比指标更该优先考虑。
- 密度不均时 K-Means 失效：它假设球形等密度，对月牙形、密度差异大、带噪数据要用 DBSCAN。
- 聚类没有标准答案：同一份数据不同算法给出不同分组，需结合业务解读而非盲目信指标。
...
上文演示了 DBSCAN，这里补充更常用的 K-Means，并用轮廓系数判断「聚得好不好」。

```python
# K-Means 聚类 + 轮廓系数评估（与上文 DBSCAN 互补）
...
print("簇中心:\n", km.cluster_centers_)
print("轮廓系数:", round(silhouette_score(X, labels), 3))   # 越接近 1 聚类越合理
```

K-Means 需要预先指定簇数 `k`，适合球形、规模相近的簇；DBSCAN 则无需指定簇数、能识别任意形状并标记噪声点。轮廓系数综合了「簇内紧密度」与「簇间分离度」，是选择 `k` 的常用依据。

## 小结

聚类是无标签下的结构发现。K-Means 简单但怕非球、怕 K 难定；DBSCAN 擅任意形状与噪声却依赖密度参数；层次聚类适合要看多粒度关系的场景。先画数据分布再选方法。

## 练习

动手检验一下自己：

1. K-Means 对初始化敏感会怎样？用什么手段缓解（如多次重启取最优）？
2. 什么数据形状下 K-Means 永远分不好、而 DBSCAN 能轻松搞定？
3. DBSCAN 的 minPts 和 eps 该怎么定，调错会分别发生什么？

## 延伸阅读

- 上一篇：[回归算法](../../机器学习/回归算法) · 下一篇：[降维算法](../降维算法)
- 返回：[本栏目总览](../)
