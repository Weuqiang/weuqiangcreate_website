...
- 显存预算算错是头号坑：不仅要算权重（如 7B FP16 ≈ 14GB），还要预留 KV cache、激活值和框架开销，实际常需权重的 1.2~2 倍。
- 量化不是免费午餐：4-bit 量化能省显存但带来可感知的质量下降，关键任务必须实测对比再决定。
- 盲目上大 batch 会 OOM：吞吐与延迟需实测权衡，调大 batch 前先确认显存余量。
- 忽略 tokenizer 差异：不同模型词表不同，同一段文本的 token 数差很多，直接影响上下文长度与计费。
