---
title: "比较运算符"
description: "比较运算符用于比较两个值的大小，然后返回一个布尔值，表示是否满足指定的条件。"
---
...
=== 要求类型与值都严格相等；== 会做类型转换。NaN 与任何值（含自身）都不等，比较应使用 Number.isNaN。
...
始终用 ===；NaN 不与自身相等，用 Number.isNaN 判断。
...
1. 解释 `[] == ![]` 为何为 true。
2. 用 Object.is 正确处理 NaN 与 -0 的边界比较。
