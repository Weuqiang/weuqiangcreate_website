...
- 掌握函数定义、五种参数形式（位置/默认/可变/关键字/仅关键字）与返回值机制
- 理解作用域与闭包的关系，会用 `lambda` 与高阶函数
- 会用递归解决可分解的问题，并识别递归的终止条件与栈深度限制
...
- [Python · 5控制流](../5控制流)
- 对照阅读：[C 语言 · 函数](../../C/6函数)（值传递 vs 对象引用传递）
- 理论支撑：[递归与分治](../../../计算机科学基础/数据结构与算法/递归与分治)

## 核心概念

函数是「命名的代码块 + 独立的名字空间」。Python 的参数传递既不是纯值传递也不是纯引用传递，而是「传对象引用」：函数内重新赋值不影响调用方，就地修改可变对象则会影响。函数本身也是对象——可以赋值、传参、返回，这是装饰器与高阶函数的基础。
...
函数封装行为、参数定义接口、返回值传递结果。牢记「传的是对象引用」和「函数是一等对象」两条，后面的装饰器、迭代器、异步都会顺理成章。

## 练习

动手检验一下自己：

1. 写一个函数，接受任意位置参数和关键字参数并原样打印，说明 `*args` / `**kwargs` 的收集规则。
2. 函数内 `lst.append(1)` 与 `lst = [1]` 对调用方的影响有何不同？为什么？
3. 为什么可变对象不适合做默认参数？给出替代方案并解释默认值的求值时机。

## 易错点

- 默认参数只求值一次：可变默认参数跨调用共享（见变量绑定陷阱），用 `None` 作哨兵替代。
- 参数顺序：`def f(a, *args, b, **kwargs)` 中关键字-only 参数 `b` 必须放在 `*args` 之后。
- 闭包捕获的是变量不是值：循环里 `lambda: i` 全部拿到最终的 `i`，用默认参数 `lambda i=i` 绑定。
- 函数忘了 `return` 会返回 `None`：调用方若继续运算会抛 `TypeError`。
- 递归没有尾递归优化：深度过大会触发 `RecursionError`，深递归应改写为迭代。

## 延伸阅读

- 上一篇：[Python · 5控制流](../5控制流)
- 下一篇：[Python · 20类与对象](../20类与对象)
- 返回 [Python 语言总览](../)
- 跨语言对照：[Go](../../Go/) · [Rust](../../Rust/) · [C](../../C/)
- 跨主题关联：[递归与分治](../../../计算机科学基础/数据结构与算法/递归与分治)（递归的构成与分治思想：Python 递归在此看复杂度与栈溢出）
