...
- 网络背景见 [网络分层与 TCP/IP](/docs/计算机科学基础/计算机网络/网络分层与TCP与IP)

## 核心概念

URL（统一资源定位符）定位网络资源，典型结构为 `scheme://host/path?query#fragment`：`scheme` 是协议（http/https），`host` 是主机，`path` 是资源路径，`query` 是查询参数，`fragment` 是页内锚点。绝对 URL 自带完整信息；相对 URL 基于当前页面基准解析。URL 中的空格、中文等需百分号编码（如 `%20`）。

本章主线：
- URL 的组成部分：scheme / host / path / query / fragment
- 绝对 URL 与相对 URL 的解析
- URL 中的保留字符与百分号编码
- `<base>` 标签对基准路径的影响
...
URL 由 scheme/host/path/query/fragment 组成；相对 URL 依赖基准解析，特殊字符需百分号编码。理解它才能正确处理站内跳转与外部链接。

## 练习

动手检验一下自己：

1. 拆解 `https://example.com/a/b?x=1#top` 的各部分。
2. 解释相对 URL `../img/1.png` 相对当前页如何解析。
3. 把含中文的查询参数写成合法的百分号编码形式。
