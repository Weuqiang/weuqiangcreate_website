---
title: "包装对象"
description: "对象是 JavaScript 语言最主要的数据类型，三种原始类型的值——数值、字符串、布尔值——在一"
---
...
- [类型概览](/docs/软件工程/Web前端/JavaScript/types/general)

## 核心概念

当对原始值调用属性或方法时，JS 会临时用对应包装对象（String/Number/Boolean）装箱，调用后立即丢弃。这就是 'a'.toUpperCase() 能工作的原因。
...
原始值调用方法时自动装箱为临时包装对象；不要长期持有包装对象。

## 练习

动手检验一下自己：

1. 解释 'a'.toUpperCase() 为何不会报错。
2. 对比原始字符串与 new String('a') 的类型。
