...
sidebar_position: 0
title: 分类算法
description: "KNN、朴素贝叶斯与支持向量机三种分类算法：距离投票、拉普拉斯平滑防零概率、核函数与数据标准化，配 sklearn 示例、准确率评估与过拟合学习曲线。"
---
...
- 理解分类的本质是学习「输入空间到类别」的映射与决策边界
- 掌握逻辑回归、kNN、决策树、SVM 四类代表方法的适用形状
- 能按数据规模、维度与可解释性选分类器
...
- 先读 [逻辑回归](../逻辑回归) 作为线性分类的入口
- 线性代数基础见 [向量空间与基](/docs/数学基础/线性代数/向量空间与基)

## 核心概念

分类算法都在做同一件事：找一个把不同类别分开的曲面（决策边界）。线性模型画直线、树模型画阶梯、SVM 找最宽间隔的超平面、kNN 干脆不训练只靠邻居投票。选哪种，取决于你的数据是不是线性可分、维度高不高、要不要解释得清。没有万能分类器，只有匹配数据形状的那个。
...
把分类流程串成一个完整闭环：划分数据集 → 训练 → 用 `classification_report` 与混淆矩阵全面评估。
...
print(classification_report(y_te, y_pred))   # 精确率/召回率/F1 一目了然
print("混淆矩阵:\n", confusion_matrix(y_te, y_pred))
```

`classification_report` 同时给出每个类别的精确率、召回率与 F1，比单看准确率更能暴露「某类被长期误判」的问题；混淆矩阵则直接指出「哪两类容易混淆」。

## 小结

分类器的核心差异在「决策边界长什么样」。先画数据的散点看形状，再选边界：线性可分时逻辑回归最稳，形状复杂时树或核方法更准，要简单可解释时 kNN 也够用。

## 易错点

- 特征未统一量纲：树模型还好，但距离类（KNN、SVM、带正则的逻辑回归）对尺度极敏感，必须先标准化/归一化。
- 类别不平衡直接拉垮：少数类被淹没，准确率虚高；要用类别权重、重采样或看 PR 曲线。
- 维度太高害死距离度量：高维下样本彼此距离趋同（维度灾难），KNN 失效，需先降维或选树模型。
- 把测试集当调参依据：用测试集选特征/超参会乐观偏差，须用交叉验证或独立验证集。

## 练习

动手检验一下自己：

1. 什么数据形状下逻辑回归注定失败、而决策树轻松搞定？
2. kNN 几乎不训练，为什么预测时反而可能很慢？
3. 高维稀疏数据（如文本）下，你会避开哪类分类器？为什么？
