...
- 命令与基础概念：[开发工具链 · Docker](../../../软件工程/开发工具链/Docker)
- 理论支撑：[操作系统 · 进程、线程与调度](../../../计算机科学基础/操作系统/进程线程与调度)
...
- 把镜像当虚拟机：装全套、不分层，镜像巨大、启动慢、分发贵。
- 以 root 运行容器：一旦逃逸直接拿到宿主机权限；应改用非 root 用户。
- 把密钥/配置烤进镜像：换环境就要重建；用环境变量或 secret 挂载。
- 基础镜像不固定版本：用 latest 漂移，今天能跑明天挂，构建不可复现。
- 容器无健康检查/无资源限制：OOM 或被调度器误杀，问题难定位。
