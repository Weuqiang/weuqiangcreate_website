...
- 理解数组的连续内存布局、下标访问与「数组名即首地址」
- 掌握指针的定义、解引用、指针算术与多级指针
- 弄清数组/指针作为函数参数时的退化规则
...
数组是一组相同类型数据的连续内存；指针是「存放地址的变量」。二者天然绑定：数组名在多数语境下退化为指向首元素的指针，于是「下标」与「指针偏移」是同一件事的两种写法。
...
C 的二维数组在内存中是**行主序（row-major）连续存储**的：先排满第 0 行所有元素，再排第 1 行，依次类推。因此 `int a[3][4]` 在内存里等价于长度为 12 的一维数组，元素顺序为 `a[0][0], a[0][1], ..., a[0][3], a[1][0], ...`。

下标访问 `a[i][j]` 在编译期会被翻译成指针算术：`a[i][j]` 等价于 `*(*(a + i) + j)`。其中 `a + i` 跳过 `i` 行（每行 `N` 个 `int`，实际偏移 `i * N * sizeof(int)`），`*(a + i)` 得到第 `i` 行的首地址（即 `a[i]`），再 `+ j` 定位到第 `j` 列。理解这点后，可以用**行指针** `int (*p)[N]`（指向「含 N 个 int 的数组」的指针）来遍历，此时 `p + 1` 正好跳一整行：
...
int main(void) {
    ...
    // 行指针：p 指向「含 4 个 int 的数组」，p+1 跳一整行
    int (*p)[4] = a;
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 4; j++) {
            // 三种写法等价：a[i][j]、*(*(a+i)+j)、*(*(p+i)+j)
            printf("%2d ", *(*(p + i) + j));
        }
        printf("\n");
    }
...
`int (*p)[N]` 里的括号不能省：写成 `int *p[N]` 会变成「含 N 个 `int*` 的数组」，两者类型完全不同。

为什么**按行遍历比按列遍历快**？因为内存连续，CPU 会按缓存行（cache line，通常 64 字节）预取相邻数据。按行访问 `a[i][j]`（j 在内存中相邻）能命中已预取的数据，缓存命中率高；按列访问 `a[j][i]` 每次都跳到相隔一整行的位置，频繁触发缓存未命中（cache miss），需要去更慢的内存取数，性能差距在大型数组上可达数倍。
...
* 生活中的地址:
  ![什么是地址示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/9485373b7cb46f41f6c996303419ba4c.png)
* 内存地址:
  ![内存地址示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/e46d975be9e95178136726209b2c237f.png)
...
![指针基本概念示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/13eb965757dd130d813a953506335b89.png)
...
- 内存单元的指针(地址)和内存单元的内容是两个不同的概念。
  ![什么是指针示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/540000371e0da0fb105248642ef9db24.png)

## 什么是指针变量

- 在 C 语言中,允许用一个变量来存放其它变量的地址, 这种专门用于存储其它变量地址的变量, 我们称之为指针变量
  ![什么是指针变量示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/02442ae9685e56146470369e56ca18ab.png)
- 示例:
...
![什么是指针变量示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/8b7b4d0976ad6e192e3ad6e26282fa8a.png)
...
* 指针类型说明,即定义变量为一个指针变量;
* 指针变量名;
  ![指针变量名示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/c051703dcae8535c264ef47c20881b53.png)
...
* 多个指针变量可以指向同一个地址
  ![多个指针变量指向同一地址示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/d4c9b29d8ddb808d1a12252fdcdf538a.png)
...
* 所以指针必须初始化才可以访问其所指向存储区域
  ![野指针访问你不该访问数据示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/e38cdad483acddca8cfcd85e7ac0be78.png)
...
- 在同一种编译器环境下,一个指针变量所占用的内存空间是固定的。
  ![指针类型示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/a95f575889111ad8d64cc2c1aa8ecb1e.png)
...
* 所以指针变量需要它所指向的数据类型告诉它要访问多少个字节存储空间
  ![指针类型示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/af1fd05a523d9ba4e196e326f537589b.png)
...
![二级指针示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/af331163ce03df7573810fc05ceb03c6.png)
...
* 只要一个指针变量保存了数组元素的地址, 我们就称之为数组元素指针
  ![数组元素指针示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/69b98c6401a04bf8fce52a78c80b9565.png)
...
![指针访问数组元素示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/13c0dd997ce619b1249220ce09610ad4.png)
...
![自减运算,如 p--,--p示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/6c563fff4c62724055337604c2d31623.png)
...
![指针法, \(p+i)形式示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/e00312fbc775c04e2a449d7d8d473eb9.png)
...
![字符串指针指向字符串示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/cb21fae540cb3290bbc41e7e223d3a41.png)
...
- 格式: `struct 结构体名 结构体变量名;`
  ![定义结构体变量示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/a19ef99bede1b3b0d689959881390bd0.png)
...
- 在 stu 中嵌套存储 Date 结构体内容
  ![结构体嵌套定义示意图](https://images.weserv.nl/?url=https://img-blog.csdnimg.cn/img_convert/de4aedd768012888dc7b2e70a744c86d.png)
...
本章是 C 的心脏：数组连续存储、指针保管地址，二者通过 `a[i] == *(a+i)`  unify。重点是二维数组的行主序、指针算术，以及「数组名作参数退化为指针」带来的 `sizeof` 失效。

## 练习

动手检验一下自己：

1. 为什么二维数组可以省略行数但不能省略列数？
2. 解释 `int (*p)[4]` 与 `int *p[4]` 的类型差异。
3. 用指针算术（而非下标）遍历一个 `int` 数组并求和。

## 易错点

- 数组越界不会报错：`int a[5]` 访问 `a[5]` 只是读写了相邻内存，程序往往还能继续跑，直到很久之后才以完全无关的方式崩溃，排查起来极其痛苦。
- 把数组名完全等同于指针：`sizeof(数组名)` 是整个数组的字节数，`sizeof(指针)` 只有 4 或 8；数组名还是不可修改的左值，`a = b;` 直接编译不过。
- 数组传参后长度信息丢失：形参写成 `int a[]` 也只是 `int *`，函数内部无法用 `sizeof` 求元素个数，必须额外传一个长度参数。
- 指针未初始化就解引用：野指针指向随机地址，写入后可能只是悄悄破坏了别的变量而不立即崩溃；定义时先置为 `NULL`，使用前判断。
- 指针算术的步长是「元素大小」而非字节：`int *p` 做 `p + 1` 实际前进 4 个字节，误按字节计算会跳错位置；对 `void *` 做算术是编译器扩展，不可移植。
- 二维数组与指针数组混为一谈：`int a[3][4]` 是一整块连续内存，`int *a[3]` 是三个各自独立的指针；把前者传给形参 `int **` 会按错误的方式寻址，读到垃圾数据。

## 易错点

- **数组越界无检查**：C 不检查边界，越界读写会踩到相邻内存，表现为偶发的诡异崩溃。
- **数组名与指针的差异**：多数上下文数组名退化为指针，但 `sizeof(arr)` 在数组上得到总字节数、在指针上只是指针大小，混用会算错。
- **野指针/未初始化指针**：指针未赋值就解引用，行为完全未定义，可能瞬间段错误。
- **指针算术按类型步进**：`p + 1` 移动的字节数等于 `sizeof(*p)`，误算偏移是常见错。
- **字符串忘留 `\0`**：为字符串分配空间时忘记多留 1 字节终止符，导致读到越界。
- **`p++` 与 `(*p)++` 混淆**：前者移动指针，后者让指针所指对象自增，语义完全不同。

## 延伸阅读

- 上一篇：[`6函数`](../6函数)
- 下一篇：[`9字符串和字符串函数`](../9字符串和字符串函数)
- 返回 [C 语言总览](../)
- 对照阅读：[Go 语言](../../Go/)、[Rust 语言](../../Rust/)、[Python 语言](../../Python/)
- 跨主题关联：[Rust 借用与生命周期](../../../编程语言/Rust/所有权/借用与生命周期)（C 指针不检查生命周期，Rust 编译期兜底）
- 跨主题关联：[Java 引用与对象](../../../编程语言/Java/语言基础/面向对象与核心语法)（"引用 vs 指针"：GC 托管 vs 手动管理）
- 跨主题关联：[内存管理·虚拟地址与指针](../../../计算机科学基础/操作系统/内存管理)（指针在 OS 层的语义：虚拟地址 ≠ 物理地址）
- 跨主题关联：[数组与链表](../../../计算机科学基础/数据结构与算法/数组与链表)（链表节点 = 值 + 指针，数据结构层的指针应用）
